...
final class Loader {
	...
	public function view($template, $data = array()) {
		...
		if (file_exists($file)) {
			...
			if(!defined('DIR_CATALOG')) {
				$this->shortcodes = $this->registry->get('shortcodes');
				$this->document = $this->registry->get('document');
				
				$this->page = $this->shortcodes->doShortCode(file_get_contents($file));
				
				if (basename($file) == 'header.tpl') {
	...
	public function style($code, $data = array()){
		$this->db = $this->registry->get('db');
		
		$style_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "block` WHERE `code`='".$this->db->escape(strtolower($code))."'");
		
		if ($style_query->num_rows) {
			extract($data);

			ob_start();

			 eval('?>' . $style_query->row['html'] . '<?php ');

			$output = ob_get_contents();

			ob_end_clean();
		} else {
			trigger_error('Error: Could not load style ' . $code . '!');
			exit();
		}
