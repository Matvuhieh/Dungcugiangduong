...
final class MSSQL {
	...
	public function __construct($hostname, $staffname, $password, $database, $port = '1433') {
		if (!$this->link = mssql_connect($hostname. ':' . $port, $staffname, $password)) {
			exit('Error: Could not make a database connection using ' . $staffname . '@' . $hostname);
		}
