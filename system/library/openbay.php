...
final class Openbay {
	...
	public function putStockUpdateBulk($instrument_id_array, $end_inactive = false) {
		 ...
		 * putStockUpdateBulk
		 *
		 * Takes an array of instrument id's where stock has been modified
		 *
		 * @param $instrument_id_array
		 */

		foreach ($this->installed_markets as $market) {
			if ($this->config->get($market . '_status') == 1) {
				$this->{$market}->putStockUpdateBulk($instrument_id_array, $end_inactive);
			}
		}
	}
	...
	public function getTaxRates($tax_class_id) {
		...
		$tax_query = $this->db->query("SELECT
				...
				INNER JOIN " . DB_PREFIX . "tax_rate_to_user_group tr2cg ON (tr2.tax_rate_id = tr2cg.tax_rate_id)
				...
				AND tr2cg.user_group_id = '" . (int)$this->config->get('config_user_group_id') . "'
	...
	public function newOrderAdminNotify($order_id, $order_status_id) {
		...
		$order_info = $this->model_borrow_order->getOrder($order_id);
		...
		$text .= $language->get('text_new_instruments') . "\n";

		foreach ($order_instrument_query->rows as $instrument) {
			$text .= $instrument['quantity'] . 'x ' . $instrument['name'] . ' (' . $instrument['model'] . ') ' . html_entity_decode($this->currency->format($instrument['total'] + ($this->config->get('config_tax') ? ($instrument['tax'] * $instrument['quantity']) : 0), $order_info['currency_code'], $order_info['currency_value']), ENT_NOQUOTES, 'UTF-8') . "\n";

			$order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_instrument_id = '" . (int)$instrument['order_instrument_id'] . "'");
	...
	public function getInstrumentModelNumber($instrument_id, $sku = null) {
		if($sku != null) {
			$qry = $this->db->query("SELECT `sku` FROM `" . DB_PREFIX . "instrument_option_variant` WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `sku` = '" . $this->db->escape($sku) . "'");
			...
			$qry = $this->db->query("SELECT `model` FROM `" . DB_PREFIX . "instrument` WHERE `instrument_id` = '" . (int)$instrument_id . "' LIMIT 1");
	...
	public function getStaffByEmail($email) {
		$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "user` WHERE `email` = '" . $this->db->escape($email) . "'");
	...
	public function getInstrumentOptions($instrument_id) {
		...
		foreach ($instrument_option_query->rows as $instrument_option) {
			if ($instrument_option['type'] == 'select' || $instrument_option['type'] == 'radio' || $instrument_option['type'] == 'checkbox' || $instrument_option['type'] == 'image') {
				...
				$instrument_option_value_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "instrument_option_value pov LEFT JOIN " . DB_PREFIX . "option_value ov ON (pov.option_value_id = ov.option_value_id) LEFT JOIN " . DB_PREFIX . "option_value_description ovd ON (ov.option_value_id = ovd.option_value_id) WHERE pov.instrument_option_id = '" . (int)$instrument_option['instrument_option_id'] . "' AND ovd.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY ov.sort_order");
				...
				$instrument_option_data[] = array(
					...
					'name'              => $instrument_option['name'],
					'type'              => $instrument_option['type'],
					'option_value'      => $instrument_option['value'],
					'required'          => $instrument_option['required']
				);
			}
		}
	...
	public function getOrderInstruments($order_id) {
		$order_instruments = $this->db->query("SELECT `instrument_id`, `order_instrument_id` FROM `" . DB_PREFIX . "order_instrument` WHERE `order_id` = '" . (int)$order_id . "'");

		if($order_instruments->num_rows > 0) {
			return $order_instruments->rows;
		} else {
			return array();
		}
	}

	public function getOrderInstrumentVariant($order_id, $instrument_id, $order_instrument_id) {
		$this->load->model('module/openstock');

		$order_option_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_option WHERE order_id = '" . (int)$order_id . "' AND order_instrument_id = '" . (int)$order_instrument_id . "'");

		if ($order_option_query->num_rows) {
			$options = array();

			foreach ($order_option_query->rows as $option) {
				$options[] = $option['instrument_option_value_id'];
			}

			return $this->model_module_openstock->getVariantByOptionValues($options, $instrument_id);
		}
	}
}