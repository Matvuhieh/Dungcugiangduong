	...
	public function send() {
		...
		if ($this->protocol == 'mail') {
			...
			if (!$handle) {
				...
				if (!empty($this->smtp_staffname)  && !empty($this->smtp_password)) {
					fputs($handle, 'EHLO ' . getenv('SERVER_NAME') . "\r\n");
					...
					fputs($handle, base64_encode($this->smtp_staffname) . "\r\n");
					...
					if (substr($reply, 0, 3) != 334) {
						throw new \Exception('Error: Staffname not accepted from server!');
					}
