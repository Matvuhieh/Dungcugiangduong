...
class Mfilter_Plus {
	...
	public function baseJoin( array $skip = array() ) {
		...
		/*if( ! in_array( 'pmv', $skip ) && ( $this->_values['attribute'] || $this->_values['option'] || $this->_values['filter'] ) ) {
			$sql .= sprintf("
				INNER JOIN
					`" . DB_PREFIX . "instrument_mfilter_values` AS `pmv`
				ON
					`pmv`.`instrument_id` = `p`.`instrument_id`
			" );
		}*/
	...
	public function optionsToSQL( $join, $options, & $conditionsIn = NULL, & $conditionsOut = NULL ) {
					...
					if( isset( $this->_values['option'][$val3] ) ) {
						...
						if( ! empty( $this->_settings['stock_for_options_plus'] ) ) {
							if( ! empty( $this->_settings['in_stock_default_selected'] ) || in_array( $this->inStockStatus(), $this->stockStatusValues() ) ) {
								//$ids[] = $val3;
								$sql3 .= " AND ( SELECT	SUM(`quantity`) FROM `" . DB_PREFIX . "instrument_option_value` AS `pov` WHERE `p`.`instrument_id` = `pov`.`instrument_id` AND `option_value_id` = '" . $val3 . "' ) > 0";
		...
		/*if( $ids && $conditionsIn !== NULL ) {
			$conditionsIn[] = "`p`.`instrument_id` IN(
				SELECT 
					`pov`.`instrument_id` 
				FROM 
					`" . DB_PREFIX . "instrument_option_value` AS `pov` 
				WHERE 
	...
	public function install() {
		...
		if( $this->_addColumn( 'instrument', 'mfilter_values', 'TEXT NOT NULL DEFAULT ""' ) ) {
			$this->db->query("ALTER TABLE `" . DB_PREFIX . "instrument` ADD FULLTEXT(`mfilter_values`)");
			$this->db->query("ALTER TABLE `" . DB_PREFIX . "mfilter_values` ADD UNIQUE( `type`, `value`, `value_id` )");
			
			if( $this->_addColumn( 'instrument', 'mfilter_tags', 'TEXT NOT NULL DEFAULT ""' ) ) {
				$this->db->query("ALTER TABLE `" . DB_PREFIX . "instrument` ADD FULLTEXT(`mfilter_tags`)");
	...
	public function installprogress() {
		$steps	= array( 'attribute', 'option', 'instrument', 'tag' );
		...
		if( empty( $this->session->data['mfilter_plus_install'] ) ) {
			...
			$this->db->query( "UPDATE `" . DB_PREFIX . "instrument` SET `mfilter_values` = ''" );
			
			$this->db->query( "TRUNCATE `" . DB_PREFIX . "mfilter_tags`" );
			$this->db->query( "UPDATE `" . DB_PREFIX . "instrument` SET `mfilter_tags` = ''" );
		}
		...
		switch( $steps[$step] ) {
			case 'attribute' : {
				if( empty( $this->_settings['attribute_separator'] ) ) {
					$sql = "SELECT REPLACE(REPLACE(TRIM(`text`), '\r', ''), '\n', '') AS `val`, `attribute_id` AS `val_id` FROM `" . DB_PREFIX . "instrument_attribute` GROUP BY `val`, `val_id`";
				} else {
					$sql = "SELECT `text` AS `val`, `attribute_id` AS `val_id` FROM `" . DB_PREFIX . "instrument_attribute` GROUP BY `val`, `val_id`";
				}
				$rows = $this->db->query("SELECT COUNT(*) AS `c` FROM(SELECT * FROM `" . DB_PREFIX . "instrument_attribute` GROUP BY `text`) AS t");
				$rows = $rows->row['c'];
			...
			case 'tag' : {
				$sql = "SELECT `tag` AS `val` FROM `" . DB_PREFIX . "instrument_description`";
				$rows = $this->db->query("SELECT COUNT(*) AS `c` FROM `" . DB_PREFIX . "instrument_description`");
				$rows = $rows->row['c'];
		...
		if( $sql ) {
			...
			$rows = $this->db->query("SELECT COUNT(*) AS `c` FROM `" . DB_PREFIX . "instrument`")->row['c'];
			$limit = 10;
			$_rows = 0;
			
			foreach( $this->db->query( "SELECT * FROM `" . DB_PREFIX . "instrument` LIMIT " . ( $limit * $idx ) . ', ' . $limit )->rows as $row ) {
				$this->updateInstrument( $row['instrument_id'] );
			}
	...
	public function uninstall() {
		...
		$this->_removeColumn( 'instrument', 'mfilter_values' );
	...
	public function updateInstrument( $instrument_id ) {
		$attribs		= $this->db->query( "SELECT * FROM `" . DB_PREFIX . "instrument_attribute` WHERE `instrument_id` = " . (int) $instrument_id );
		$options		= $this->db->query( "SELECT * FROM `" . DB_PREFIX . "instrument_option_value` WHERE `instrument_id` = " . (int) $instrument_id );
		$filters		= $this->hasFilters() ? $this->db->query( "SELECT * FROM `" . DB_PREFIX . "instrument_filter` WHERE `instrument_id` = " . (int) $instrument_id ) : NULL;
		$tags			= $this->db->query( "SELECT instrument_id, tag FROM `" . DB_PREFIX . "instrument_description` WHERE `instrument_id` = " . (int) $instrument_id );
		$before			= array();
		$after			= array();
		$beforeTag		= array();
		$afterTag		= array();
		...
		$instrument = $this->db->query("SELECT * FROM `" . DB_PREFIX . "instrument` WHERE `instrument_id` = " . (int) $instrument_id )->row;
		
		$before = explode( ',', $instrument['mfilter_values'] );
		$beforeTag = explode( ',', $instrument['mfilter_tags'] );
		...
		$this->db->query(sprintf("
			...
			WHERE
				`instrument_id` = %s
		", implode( ',', array_unique( $after ) ), $instrument_id));
		...
		$this->db->query(sprintf("
			...
			WHERE
				`instrument_id` = %s
		", implode( ',', array_unique( $afterTag ) ), $instrument_id));
		...
		if( $diff ) {
			foreach( $diff as $id ) {
				if( ! $this->db->query(sprintf("SELECT * FROM `" . DB_PREFIX . "instrument` WHERE FIND_IN_SET( '%s', `mfilter_values` ) LIMIT 1", $id))->num_rows ) {
		...
		if( $diffTag ) {
			foreach( $diffTag as $id ) {
				if( ! $this->db->query(sprintf("SELECT * FROM `" . DB_PREFIX . "instrument` WHERE FIND_IN_SET( '%s', `mfilter_tags` ) LIMIT 1", $id))->num_rows ) {
