...
final class Etsy {
	...
	public function getLinks($instrument_id, $status = 0, $limit = null) {
		$this->log('getLinks() - Instrument_id: ' . $instrument_id . ' status: ' . $status . ' limit:' . $limit);
		...
		$qry = $this->db->query("SELECT `el`.*, `p`.`quantity` FROM `" . DB_PREFIX . "etsy_listing` `el` LEFT JOIN `" . DB_PREFIX . "instrument` `p` ON `el`.`instrument_id` = `p`.`instrument_id` WHERE `el`.`instrument_id` = '" . (int)$instrument_id . "' AND `el`.`status` = '" . (int)$status . "' ORDER BY `el`.`created` DESC" . $sql_limit);
	...
	public function getLinkedInstrument($etsy_item_id) {
		$qry = $this->db->query("SELECT `p`.`quantity`, `p`.`instrument_id`, `p`.`model`, `el`.`etsy_listing_id`, `el`.`status` AS `link_status` FROM `" . DB_PREFIX . "etsy_listing` `el` LEFT JOIN `" . DB_PREFIX . "instrument` `p` ON `p`.`instrument_id` = `el`.`instrument_id` WHERE `el`.`etsy_item_id` = '" . (int)$etsy_item_id . "' AND `el`.`status` = 1");
	...
	public function updateAllStock($instrument_id, $new_stock) {
		/**
		 * This will update all linked listings with the set stock amount
		 */

		/** @var loop over linked items $response
		$response = $this->openbay->etsy->call('v1/etsy/instrument/listing/'.(int)$data['listing_id'].'/image/', 'POST', $data);
	...
	public function updateListingStock($etsy_item_id, $new_stock, $status) {
		if ($new_stock > 0) {
			...
			$response = $this->call('v1/etsy/instrument/listing/' . (int)$etsy_item_id . '/updateStock/', 'POST', array('quantity' => $new_stock, 'state' => $status));
			...
			$response = $this->call('v1/etsy/instrument/listing/' . (int)$etsy_item_id . '/inactive/', 'POST');
	...
	public function deleteInstrument($instrument_id) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "etsy_listing` WHERE `instrument_id` = '" . (int)$instrument_id . "'");
	}
	...
	public function instrumentUpdateListen($instrument_id, $data = array()) {
		...
		$links = $this->getLinks($instrument_id, 1);
	...
	public function orderDelete($order_id) {
		if(!$this->orderFind($order_id)) {
			$query = $this->db->query("SELECT `p`.`instrument_id` FROM `" . DB_PREFIX . "order_instrument` `op` LEFT JOIN `" . DB_PREFIX . "instrument` `p` ON `op`.`instrument_id` = `p`.`instrument_id` WHERE `op`.`order_id` = '" . (int)$order_id . "'");

			if($query->num_rows > 0) {
				foreach ($query->rows as $instrument) {
					$this->instrumentUpdateListen((int)$instrument['instrument_id']);
				}
			}
		}
	}
	...
	public function putStockUpdateBulk($instrument_id_array, $end_inactive) {
	...
	public function getEtsyItem($listing_id) {
		$response = $this->openbay->etsy->call('v1/etsy/instrument/listing/' . $listing_id . '/', 'GET');
