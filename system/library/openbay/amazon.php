...
class Amazon {
	...
	public function instrumentUpdateListen($instrument_id, $data = array()) {
		...
		$logger->write('instrumentUpdateListen (' . $instrument_id . ')');
	...
	public function bulkUpdateOrders($orders) {
		...
		foreach ($orders as $order) {
			...
			$amazon_order_instruments = $this->model_openbay_amazon->getAmazonOrderedInstruments($order['order_id']);

			$instruments = array();

			foreach ($amazon_order_instruments as $amazon_order_instrument) {
				$instruments[] = array(
					'amazon_order_item_id' => $amazon_order_instrument['amazon_order_item_id'],
	...
	public function updateOrder($order_id, $order_status_string, $courier_id = '', $courier_from_list = true, $tracking_no = '') {
		...
		$this->load->model('openbay/amazon');
		$amazon_order_instruments = $this->model_openbay_amazon->getAmazonOrderedInstruments($order_id);
		...
		foreach ($amazon_order_instruments as $instrument) {
			$new_order_item = $order_items_node->addChild('OrderItem');
			$new_order_item->addChild('ItemId', htmlspecialchars($instrument['amazon_order_item_id']));
	...
	public function putStockUpdateBulk($instrument_id_array, $end_inactive = false){
		...
		foreach($instrument_id_array as $instrument_id) {
			$amazon_rows = $this->getLinkedSkus($instrument_id);
			foreach($amazon_rows as $amazon_row) {
				...
				if(!empty($instrument_row)) {
					if($end_inactive && $instrument_row['status'] == '0') {
						...
						$quantity_data[$amazon_row['amazon_sku']] = $instrument_row['quantity'];
					}
				}
			}
		}
	...
	public function getLinkedSkus($instrument_id, $var='') {
		return $this->db->query("SELECT `amazon_sku` FROM `" . DB_PREFIX . "amazon_instrument_link` WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `var` = '" . $this->db->escape($var) . "'")->rows;
	}
	...
	public function deleteInstrument($instrument_id){
		$this->db->query("DELETE FROM `" . DB_PREFIX . "amazon_instrument_link` WHERE `instrument_id` = '" . (int)$instrument_id . "'");
	}
