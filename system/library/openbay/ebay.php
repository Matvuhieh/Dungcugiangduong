...
final class Ebay {
	...
	public function getEbayItemId($instrument_id) {
		$this->log('getEbayItemId() - Instrument ID: ' . $instrument_id);

		$qry = $this->db->query("SELECT `ebay_item_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `status` = '1' LIMIT 1");
	...
	public function getEndedEbayItemId($instrument_id) {
		$this->log('getEndedEbayItemId() - ID: ' . $instrument_id);

		$qry = $this->db->query("SELECT `ebay_item_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `status` = '0' ORDER BY `ebay_listing_id` DESC LIMIT 1");
	...
	public function removeItemByInstrumentId($instrument_id) {
		$this->log('removeItemByInstrumentId() - ID: ' . $instrument_id . '');

		$this->db->query("UPDATE `" . DB_PREFIX . "ebay_listing` SET `status` = '0' WHERE `instrument_id` = '" . (int)$instrument_id . "'");

		$this->db->query("DELETE FROM `" . DB_PREFIX . "ebay_stock_reserve` WHERE `instrument_id` = '" . (int)$instrument_id . "'");
	}

	public function deleteInstrument($instrument_id) {
		$this->log('deleteInstrument() - ID: ' . $instrument_id);

		$this->db->query("DELETE FROM `" . DB_PREFIX . "ebay_listing` WHERE `instrument_id` = '" . (int)$instrument_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "ebay_stock_reserve` WHERE `instrument_id` = '" . (int)$instrument_id . "'");
	}
	 ...
	 * @return array ([instrument id] = ebay item id)
	 */
		$this->log('getLiveListingArray()');

		$qry = $this->db->query("SELECT `instrument_id`, `ebay_item_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `status` = '1'");

		$data = array();
		if ($qry->num_rows > 0) {
			foreach ($qry->rows as $row) {
				$data[$row['instrument_id']] = $row['ebay_item_id'];
			}
		}
	...
	public function getEndedListingArray() {
		...
		$qry = $this->db->query("SELECT e.* FROM (SELECT `instrument_id`, MAX(`ebay_listing_id`) as `ebay_listing_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `status` = 0 GROUP BY `instrument_id`) `a` INNER JOIN `" . DB_PREFIX . "ebay_listing` `e` ON (`e`.`ebay_listing_id` = `a`.`ebay_listing_id`)");
	...
	public function getLiveInstrumentArray() {
		/**
		* Returns the list of linked items with eBay from the database
		* @return array ([ebay item id] = instrument id)
		*/
		$qry = $this->db->query("SELECT `instrument_id`, `ebay_item_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `status` = '1'");
		...
		if ($qry->num_rows) {
			foreach ($qry->rows as $row) {
				$data[$row['ebay_item_id']] = $row['instrument_id'];
			}
		}
	...
	public function ebayManageStockReduce($instrument_id, $sku = null) {
		/**
		* Gets the instrument info from an ID and sends to ebay update method.
		*/
		$this->log('ebayManageStockReduce() - Is stock update needed (Item ID: ' . $instrument_id . ',SKU: ' . $sku . ')');

		if (!empty($instrument_id)) {
			if ($sku == null) {
				$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "instrument` WHERE `instrument_id` = '" . (int)$instrument_id . "' LIMIT 1");
				$this->log('ebayManageStockReduce() - Send item ID: "' . $instrument_id . '", Stock: "' . $query->row['quantity'] . '" to decideEbayStockAction()');
				$this->decideEbayStockAction($instrument_id, $query->row['quantity'], $query->row['subtract']);
			} else {
				$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "instrument_option_variant` WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `sku` = '" . $this->db->escape($sku) . "' LIMIT 1");
				$this->log('ebayManageStockReduce() - Send item ID: ' . $instrument_id . ', VAR: ' . $sku . ', passing ' . $query->row['stock'] . ' to decideEbayStockAction()');
				$this->decideEbayStockAction($instrument_id, $query->row['stock'], $query->row['subtract'], $sku);
			}
		}
	}
	...
	private function getManageRecord($manage_id) {
		$this->log('getManageRecord() - Get ebay manage record ID: ' . $manage_id);
		return $this->call('order/getSmpRecord/', array('id' => $manage_id));
	}
	...
	public function disableInstrument($instrument_id) {
		$this->db->query("UPDATE `" . DB_PREFIX . "instrument` SET `status` = 0 WHERE `instrument_id` = '" . (int)$instrument_id . "' LIMIT 1");
	}

	public function disableVariant($instrument_id, $sku) {
		$this->db->query("UPDATE `" . DB_PREFIX . "instrument_option_variant` SET `active` = 0 WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `sku` = '" . $this->db->escape($sku) . "' LIMIT 1");
	}
	...
	public function putStockUpdate($item_id, $stock, $sku = null) {
		...
		$instrument_id = $this->getInstrumentId($item_id);
		$reserve    = $this->getReserve($instrument_id, $item_id, (($sku != null) ? $sku : ''));
			...
			if ($sku == null) {
				...
				if ($stock <= 0) {
					if ($this->config->get('ebay_disable_nostock') == 1) {
						$this->disableInstrument($instrument_id);
					}
				...
				if ($stock <= 0) {
					if ($this->config->get('ebay_disable_nostock') == 1) {
						$this->disableVariant($instrument_id, $sku);
					}
				}
	...
	public function putStockUpdateBulk($instrument_id_array, $end_inactive = false) {
		// We know is that these instrument ID's have been modified. They should only be passed if the stock has changed so we can assume this.
		...
		foreach ($instrument_id_array as $instrument_id) {
			if (array_key_exists((int)$instrument_id, $live_data)) {
				//instrument has been passed and is linked to active item
				$linked_items[] = array('instrumentId' => (int)$instrument_id, 'itemId' => $live_data[$instrument_id]);
			}elseif (array_key_exists((int)$instrument_id, $ended_data)) {
				//instrument has been passed and is not currently active
				$linked_ended_items[] = array('instrumentId' => (int)$instrument_id, 'itemId' => $ended_data[$instrument_id]);
			} else {
				//instrument does not exist in live or ended links so has never been linked.
			}
		}
		...
		foreach ($linked_ended_items as $item) {
			if ($openstock == true) {
				$options = $this->model_module_openstock->getVariants($item['instrumentId']);
			} else {
				$options = array();
			}
			...
			if (empty($options)) {
				...
				$local_stock = $this->getInstrumentStockLevel($item['instrumentId']);

				if ((int)$local_stock['quantity'] > 0 && $local_stock['status'] == 1) {
					//instrument has stock and is enabled, so re list it.
					$reserve = $this->getReserve($item['instrumentId'], $item['itemId']);
					...
					if ($this->config->get('ebay_relistitems') == 1) {
						...
						$this->relistItem($item['itemId'], $item['instrumentId'], (int)$local_stock['quantity']);
					}
				}
			} else {
		...
		foreach ($linked_items as $item) {
			...
			if (!isset($ebay_listings[$item['itemId']])) {
				...
				if ($end_inactive == true && $local_stock['status'] == 0) {
					...
					//get any options that are set for this instrument
					if ($openstock == true) {
						$options = $this->model_module_openstock->getVariants($item['instrumentId']);
					} else {
						$options = array();
						...
						foreach ($ebay_listings[$item['itemId']]['variants'] as $ebay_variant) {
							...
							if (in_array($ebay_variant['sku'], $var_ids)) {
								...
								if ($ebay_variant['qty'] != $options[$option_id]['stock']) {
									$reserve = $this->getReserve($item['instrumentId'], $item['itemId'], $ebay_variant['sku']);
	...
	public function getInstrumentStockLevel($instrument_id, $sku = '') {
		$this->log('getInstrumentStockLevel() - ID: ' . $instrument_id . ', SKU: ' . $sku);

		if ($sku == '' || $sku == null) {
			$qry = $this->db->query("SELECT `quantity`, `status` FROM `" . DB_PREFIX . "instrument` WHERE `instrument_id` = '" . (int)$instrument_id . "' LIMIT 1");

			return array('quantity' => (int)$qry->row['quantity'], 'status' => ($qry->row['status']));
		} else {
			$qry = $this->db->query("SELECT `stock`, `active` FROM `" . DB_PREFIX . "instrument_option_variant` WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `sku` = '" . $this->db->escape($sku) . "' LIMIT 1");

			return array('quantity' => (int)$qry->row['stock'], 'status' => ($qry->row['active']));
		}
	}

	public function instrumentUpdateListen($instrument_id, $data = array()) {
		$this->log('instrumentUpdateListen(' . $instrument_id . ')');

		$item_id = $this->getEbayItemId($instrument_id);

		$instrument = $this->db->query("SELECT DISTINCT * FROM `" . DB_PREFIX . "instrument` WHERE `instrument_id` = '" . (int)$instrument_id . "' LIMIT 1")->row;

		if ($item_id != false) {
			$this->log('instrumentUpdateListen(' . $instrument_id . ') - listing found (' . $item_id . ')');

			if ($this->openbay->addonLoad('openstock') && (isset($instrument['has_option']) && $instrument['has_option'] == 1)) {
				$this->load->model('module/openstock');
				$this->load->model('tool/image');
				$this->load->model('catalog/instrument');

				$this->log('instrumentUpdateListen(' . $instrument_id . ') - Variant');

				if (!isset($data['variant'])) {
					$variants = $this->model_module_openstock->getVariants($instrument_id);
				} else {
				...
				$groups = $this->openbay->getInstrumentOptions($instrument_id);
				$variant_data['groups']  = array();
				$variant_data['related'] = array();

				foreach ($groups as $grp) {
					$t_tmp = array();
					foreach ($grp['instrument_option_value'] as $grp_node) {
						$t_tmp[$grp_node['option_value_id']] = $grp_node['name'];

						$variant_data['related'][$grp_node['instrument_option_value_id']] = $grp['name'];
					}
				...
				foreach ($variants as $option) {
					if (!empty($option['sku'])) {
						...
						$reserve = $this->getReserve($instrument_id, $item_id, $option['sku']);
						if ($reserve != false) {
							$this->log('instrumentUpdateListen() / Variant (' . $option['sku'] . ') - Reserve stock: ' . $reserve);
				...
				if (!empty($variant_data['opt'])) {
					if ($stock == true) {
						$this->log('instrumentUpdateListen() - Sending to API');
						$response = $this->call('item/reviseStockVariants', $variant_data);

						return $response;
					} else {
						$this->log('instrumentUpdateListen() - Ending item');
						$this->endItem($item_id);
					}
				}
			} else {
				$this->log('instrumentUpdateListen(' . $instrument_id . ') - Not a variant');

				$this->decideEbayStockAction($instrument_id, $instrument['quantity'], $instrument['subtract']);

				return array('msg' => 'ok', 'error' => false);
			}
		} else {
			$old_item_id = $this->getEndedEbayItemId($instrument_id);
			$this->log('instrumentUpdateListen(' . $instrument_id . ') - Standard item. Old ID: ' . $old_item_id);

			if ($old_item_id != false) {
				if ($this->openbay->addonLoad('openstock') && (isset($instrument['has_option']) && $instrument['has_option'] == 1)) {
					$this->log('instrumentUpdateListen(' . $instrument_id . ') - multi variant items relist not supported');
					/**
					 * reserved for multi variant instruments can be relisted automatically.
					 */
				} else {
					$this->log('instrumentUpdateListen(' . $instrument_id . ') - Normal, stock(' . $instrument['quantity'] . ') > 0');
					if ($instrument['quantity'] > 0) {
						if ($this->config->get('ebay_relistitems') == 1) {
							$this->relistItem($old_item_id, $instrument_id, $instrument['quantity']);
						}
					}
				}
			} else {
				$this->log('instrumentUpdateListen() - stoping, nothing found');
			}
		}
	}
	...
	public function orderStatusListen($order_id, $status_id, $data = array()) {
		...
		if ($ebay_id != false) {
			...
			$item_txn_array = $this->getManageRecord($ebay_id);
	...
	public function decideEbayStockAction($instrument_id, $qty, $subtract, $sku = null) {
		if ($subtract == 1) {
			$this->log('decideEbayStockAction() - Instrument ID: ' . $instrument_id . ', Current stock: ' . $qty);
			...
			$this->log('decideEbayStockAction() - Instrument ID: ' . $instrument_id . ' does not subtract stock');
		}
	}

	public function getInstrumentId($ebay_item, $status = 0) {
		$this->log('getInstrumentId() - Item: ' . $ebay_item);
		...
		$qry = $this->db->query("SELECT `instrument_id` FROM `" . DB_PREFIX . "ebay_listing` WHERE `ebay_item_id` = '" . $this->db->escape($ebay_item) . "'" . $status_sql . " LIMIT 1");

		if (!$qry->num_rows) {
			return false;
		} else {
			return $qry->row['instrument_id'];
		}
	}

	public function getInstrumentIdFromKey($key) {
		$qry = $this->db->query("SELECT `instrument_id` FROM `" . DB_PREFIX . "ebay_listing_pending` WHERE `key` = '" . $this->db->escape($key) . "' LIMIT 1");
	...
	public function getAllocatedStock($instrument_id) {
		$qry = $this->db->query("SELECT SUM(`qty`) AS `total` FROM `" . DB_PREFIX . "ebay_transaction` WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `order_id` = '0' LIMIT 1");
		return (int)$qry->row['total'];
	}

	public function getImages() {
		$this->log('getImages() - Getting instrument images . ');
		...
		if ($qry->num_rows) {
			foreach ($qry->rows as $img) {
				...
				if ($copy_status == true) {
					if ($img['imgcount'] == 0) {
						$this->db->query("UPDATE `" . DB_PREFIX . "instrument` SET `image` = 'catalog/" . $img['name'] . "' WHERE `instrument_id` = '" . (int)$img['instrument_id'] . "' LIMIT 1");
					} else {
						$this->db->query("INSERT INTO `" . DB_PREFIX . "instrument_image` SET `instrument_id` = '" . (int)$img['instrument_id'] . "', `image` = 'catalog/" . $this->db->escape($img['name']) . "', `sort_order` = '" . (int)$img['imgcount'] . "'");
					}
	...
	public function relistItem($item_id, $instrument_id, $qty) {
		$this->log('relistItem() - Starting relist item, ID: ' . $item_id . ', instrument: ' . $instrument_id . ', qty: ' . $qty);
		...
		if (!empty($response['ItemID'])) {
			...
			$this->createLink($instrument_id, $response['ItemID'], '');
	...
	public function createLink($instrument_id, $item_id, $variant) {
		$this->deleteInstrument($instrument_id);
		$this->removeItemByItemId($item_id);

		$this->db->query("INSERT INTO `" . DB_PREFIX . "ebay_listing` SET `instrument_id` = '" . (int)$instrument_id . "', `ebay_item_id` = '" . $this->db->escape($item_id) . "', `variant` = '" . (int)$variant . "', `status` = '1'");
	}

	public function addReserve($data, $item_id, $variant) {
		if ($variant == 1) {
			foreach ($data['opt'] as $variation) {
				$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "instrument_option_variant` WHERE `instrument_id` = '" . (int)$data['instrument_id'] . "' AND `sku` = '" . $this->db->escape($variation['sku']) . "' LIMIT 1");

				if ($query->row['stock'] != $variation['qty']) {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "ebay_stock_reserve` SET `instrument_id` = '" . (int)$data['instrument_id'] . "', `item_id` = '" . $this->db->escape($item_id) . "', `variant_id` = '" . $this->db->escape($variation['sku']) . "', `reserve` = '" . (int)$variation['qty'] . "'");
				}
			}
		} else {
			$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "instrument` WHERE `instrument_id` = '" . (int)$data['instrument_id'] . "' LIMIT 1");

			if ($query->row['quantity'] != $data['qty'][0]) {
				$this->db->query("INSERT INTO `" . DB_PREFIX . "ebay_stock_reserve` SET `instrument_id`    = '" . (int)$data['instrument_id'] . "', `item_id` = '" . $this->db->escape($item_id) . "', `variant_id` = '', `reserve` = '" . (int)$data['qty'][0] . "'");
			}
		}
	}

	public function getReserve($instrument_id, $item_id, $sku = '') {
		$this->log('getReserve()');
		$query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_stock_reserve` WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `variant_id` = '" . $this->db->escape($sku) . "' AND `item_id` = '" . $this->db->escape($item_id) . "'  LIMIT 1");
	...
	public function updateReserve($instrument_id, $item_id, $reserve, $sku = '', $variant = 0) {
		$this->log('updateReserve() - start');
		$this->log('updateReserve() - $instrument_id: ' . $instrument_id);
		...
		if ($reserve == 0) {
			$this->deleteReserve($instrument_id, $item_id, $sku);
		} else {
			if ($this->getReserve($instrument_id, $item_id, $sku) != false) {
				$this->db->query("UPDATE `" . DB_PREFIX . "ebay_stock_reserve` SET `reserve` = '" . (int)$reserve . "' WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `variant_id` = '" . $this->db->escape($sku) . "' AND `item_id` = '" . $this->db->escape($item_id) . "'  LIMIT 1");
			} else {
				if ($variant == 0) {
					$this->log('updateReserve() - not a variant');
					$this->addReserve(array('instrument_id' => $instrument_id, 'qty' => array(0 => $reserve)), $item_id, 0);
				} else {
					$this->log('updateReserve() - variant');
					$this->addReserve(array('instrument_id' => $instrument_id, 'opt' => array(array('sku' => $sku, 'qty' => $reserve))), $item_id, 1);
				}
			}
		}
	}

	public function deleteReserve($instrument_id, $item_id, $sku = '') {
		$this->log('deleteReserve()');
		$this->db->query("DELETE FROM `" . DB_PREFIX . "ebay_stock_reserve` WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `variant_id` = '" . $this->db->escape($sku) . "' AND `item_id` = '" . $this->db->escape($item_id) . "'  LIMIT 1");
	}
	...
	public function updateSettings() {
		...
		if ($this->lasterror === false) {
			...
			if (isset($response['instrument_details'])) {
				$qry = $this->db->query("SELECT * FROM `" . DB_PREFIX . "ebay_setting_option` WHERE `key` = 'instrument_details' LIMIT 1");

				if ($qry->num_rows > 0) {
					$this->db->query("UPDATE `" . DB_PREFIX . "ebay_setting_option` SET `data` = '" . $this->db->escape(serialize($response['instrument_details'])) . "', `last_updated`  = now() WHERE `key` = 'instrument_details' LIMIT 1");
					$this->log('Updated instrument_details into ebay_setting_option table');
				} else {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "ebay_setting_option` SET `key` = 'instrument_details', `data` = '" . $this->db->escape(serialize($response['instrument_details'])) . "', `last_updated`  = now()");
					$this->log('Inserted instrument_details into ebay_setting_option table');
				}
			} else {
			...
			if (isset($response['instrument_details'])) {
				...
				if ($qry->num_rows > 0) {
					$this->db->query("UPDATE `" . DB_PREFIX . "ebay_setting_option` SET `data` = '" . $this->db->escape(serialize($response['instrument_details'])) . "', `last_updated`  = now() WHERE `key` = 'instrument_details' LIMIT 1");
					$this->log('Updated instrument_details info in to ebay_setting_option table');
				} else {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "ebay_setting_option` SET `key` = 'instrument_details', `data` = '" . $this->db->escape(serialize($response['instrument_details'])) . "', `last_updated`  = now()");
					$this->log('Inserted instrument_details info in to ebay_setting_option table');
				}
			} else {
				$this->log('No instrument_details set!');
			}
		}
