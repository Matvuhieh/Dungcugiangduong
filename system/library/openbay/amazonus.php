...
class Amazonus {
	...
	public function instrumentUpdateListen($instrument_id, $data = array()) {
		...
		$logger->write('instrumentUpdateListen (' . (int)$instrument_id . ')');
		...
		if ($this->openbay->addonLoad('openstock') && (isset($instrument['has_option']) && $instrument['has_option'] == 1)) {
			...
			foreach ($variants as $variant) {
				$amazon_sku_rows = $this->getLinkedSkus($instrument_id, $variant['sku']);
			...
			$this->putStockUpdateBulk(array($instrument_id));
		}

		$logger->write('instrumentUpdateListen() exit');
	}
	...
	public function bulkUpdateOrders($orders) {
		...
		foreach ($orders as $order) {
			...
			$amazon_order_instruments = $this->model_openbay_amazonus->getAmazonusOrderedInstruments($order['order_id']);

			$instruments = array();

			foreach ($amazon_order_instruments as $amazon_order_instrument) {
				$instruments[] = array(
					'amazon_order_item_id' => $amazon_order_instrument['amazonus_order_item_id'],
					'quantity' => $amazon_order_instrument['quantity'],
				);
			}
	...
	public function updateOrder($order_id, $order_status_string, $courier_id = '', $courier_from_list = true, $tracking_no = '') {
		...
		$this->load->model('openbay/amazonus');
		$amazonus_order_instruments = $this->model_openbay_amazonus->getAmazonusOrderedInstruments($order_id);
		...
		foreach ($amazonus_order_instruments as $instrument) {
			$new_order_item = $order_items_node->addChild('OrderItem');
			$new_order_item->addChild('ItemId', htmlspecialchars($instrument['amazonus_order_item_id']));
			$new_order_item->addChild('Quantity', (int)$instrument['quantity']);
		}
	...
	public function getCategoryTemplates() {
		$result = $this->call("instrumentv2/RequestTemplateList");
		if(isset($result)) {
	...
	public function registerInsertion($data) {
		$result = $this->call("instrumentv2/RegisterInsertionRequest", $data);
		if(isset($result)) {
			return (array)json_decode($result);
		} else {
			return array();
	...
	public function insertInstrument($data) {
		$result = $this->call("instrumentv2/InsertInstrumentRequest", $data);
		if(isset($result)) {
			return (array)json_decode($result);
		} else {
			return array();
	...
	public function updateQuantities($data) {
		$result = $this->call("instrument/UpdateQuantityRequest", $data);
		if(isset($result)) {
			return (array)json_decode($result);
		} else {
			return array();
	...
	public function putStockUpdateBulk($instrument_id_array, $end_inactive = false){
		...
		foreach($instrument_id_array as $instrument_id) {
			$amazonus_rows = $this->getLinkedSkus($instrument_id);
			foreach($amazonus_rows as $amazonus_row) {
				$instrument_row = $this->db->query("SELECT quantity, status FROM `" . DB_PREFIX . "instrument` WHERE `instrument_id` = '" . (int)$instrument_id . "'")->row;

				if(!empty($instrument_row)) {
					if($end_inactive && $instrument_row['status'] == '0') {
						$quantity_data[$amazonus_row['amazonus_sku']] = 0;
					} else {
						$quantity_data[$amazonus_row['amazonus_sku']] = $instrument_row['quantity'];
					}
				}
			}
		}
	...
	public function getLinkedSkus($instrument_id, $var='') {
		return $this->db->query("SELECT `amazonus_sku` FROM `" . DB_PREFIX . "amazonus_instrument_link` WHERE `instrument_id` = '" . (int)$instrument_id . "' AND `var` = '" . $this->db->escape($var) . "'")->rows;
	}

	public function getOrderdInstruments($order_id) {
		return $this->db->query("SELECT `op`.`instrument_id`, `p`.`quantity` as `quantity_left` FROM `" . DB_PREFIX . "order_instrument` as `op` LEFT JOIN `" . DB_PREFIX . "instrument` as `p` ON `p`.`instrument_id` = `op`.`instrument_id` WHERE `op`.`order_id` = '" . (int)$order_id . "'")->rows;
	}
	...
	public function deleteInstrument($instrument_id){
		$this->db->query("DELETE FROM `" . DB_PREFIX . "amazonus_instrument_link` WHERE `instrument_id` = '" . (int)$instrument_id . "'");
	}
