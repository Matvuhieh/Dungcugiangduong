...
// -----------------------------------------------------------------------------
	...
	/**
	 * Instrument of two numbers in a Power-of-Two Galois Field
	 ...
	 * @protected
	 */
	protected function getGFInstrument($a, $b, $log, $alog, $gf) {
		if (($a == 0) OR ($b == 0)) {
			return 0;
		}
	...
	protected function getErrorCorrection($wd, $nb, $nd, $nc, $gf=256, $pp=301) {
		...
		for ($i = 1; $i <= $nc; ++$i) {
			$c[$i] = $c[($i-1)];
			for ($j = ($i - 1); $j >= 1; --$j) {
				$c[$j] = $c[($j - 1)] ^ $this->getGFInstrument($c[$j], $alog[$i], $log, $alog, $gf);
			}
			$c[0] = $this->getGFInstrument($c[0], $alog[$i], $log, $alog, $gf);
		}
		ksort($c);
		...
		for ($b = 0; $b < $nb; ++$b) {
			...
			for ($i = 0; $i < $nd; ++$i) {
				$k = ($we[0] ^ $block[$i]);
				for ($j = 0; $j < $nc; ++$j) {
					$we[$j] = ($we[($j + 1)] ^ $this->getGFInstrument($k, $c[($nc - $j - 1)], $log, $alog, $gf));
				}
			}
