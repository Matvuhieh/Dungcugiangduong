...
if ((int)$config->get('config_building') && !isset($session->data['first_access'])) {
	$session->data['first_access'] = true;
	
	header('Location: building.html', true, 302);
...
// User Group
if ($user->isLogged()) {
	$config->set('config_user_group_id', $user->getGroupId());
} elseif (isset($session->data['user']) && isset($session->data['user']['user_group_id'])) {
	// For API calls
	$config->set('config_user_group_id', $session->data['user']['user_group_id']);
} elseif (isset($session->data['guest']) && isset($session->data['guest']['user_group_id'])) {
	$config->set('config_user_group_id', $session->data['guest']['user_group_id']);
}
...
if(!$shortcodes->authentication()) exit();
...
foreach($extensions as $extension){
	...
	${$class} = new $class($registry);
	
	$shortcodes->addShortCode($class, 'index', ${$class});
}
