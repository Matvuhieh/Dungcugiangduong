<?php
class ControllerCommonJavaScript extends Controller {
	public function index() {
		header("Content-type: text/javascript; charset: UTF-8");
		
		$javascript_defaults = array(
		...
		foreach ($javascript_defaults as $javascript_default) {
			if(file_exists($javascript_default))
				$javascript .= $this->processCompress($javascript_default);
		}
		
		if(isset($this->request->get['javascript'])) {
			$scripts = explode('|', $this->request->get['javascript']);
		} else {
			$scripts = array();
		}
		...
		foreach ($scripts as $script) {
			if($script){
				$javascript .= $this->processCompress($script);
			}
		}
		
		echo $this->gZip($javascript);
	}
