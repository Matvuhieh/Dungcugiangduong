...
class ModelModuleDQuickborrow extends Model {
	private $handle;
	private $debug;
	...
	*/
	...
	public function logWrite($message){
        if(!empty($this->session->data['d_quickborrow_debug'])){
        	$this->handle = fopen(DIR_LOGS . $this->config->get('d_quickborrow_debug_file'), 'a');
	...
	*/
	...
	public function getConfigSetting($id, $config_key, $store_id, $config_file = false, $user_group_id = 0){
		...
		$result['step']['confirm']['fields']['agree']['information_id'] = $this->config->get('config_borrow_id');
		$result['step']['confirm']['fields']['agree']['error'][0]['information_id'] = $this->config->get('config_borrow_id');

		$result['step']['payment_address']['fields'] = $result['step']['payment_address']['fields'] + $this->model_d_quickborrow_custom_field->getCustomFieldsConfigData('account');
		$result['step']['payment_address']['fields'] = $result['step']['payment_address']['fields'] + $this->model_d_quickborrow_custom_field->getCustomFieldsConfigData('address');
		$result['step']['shipping_address']['fields'] = $result['step']['shipping_address']['fields'] + $this->model_d_quickborrow_custom_field->getCustomFieldsConfigData('address');
    ...
    */
    ...
    public function updateStatistic($data){
    	...
    	$this->db->query("UPDATE " . DB_PREFIX . "dqc_statistic 
    		SET user_id = '" . (int)$this->user->getId(). "', 
	    		data = '" . $this->db->escape(json_encode($this->session->data['statistic'])) . "',
	    		date_modified = NOW() 
	...
	*/
	public function getUpdateInfo($mbooth_xml, $status = 1){
		...
		$request = 'http://opencart.dreamvention.com/api/1/index.php?route=extension/check&mbooth=' . $mbooth_xml . '&store_url=' . $user_url . '&module_version=' . $current_version . '&language_code=' . $language_code . '&opencart_version=' . VERSION . '&ip='.$ip . '&status=' .$status;
	...
	public function download_extension($mbooth_xml, $filename  = false ) {
		...
		$staffAgent = 'Googlebot/2.1 (http://www.googlebot.com/bot.html)';  
		$ch = curl_init();  
		$fp = fopen($filename, "w");  
		curl_setopt($ch, CURLOPT_USERAGENT, $staffAgent);  
