<div id="pro-mega-menu-container"> <a href="javascript:void(0);" class="responsive-menu-controller"><?php echo $responsive_title;?></a>
  <ul id='pro-mega-menu'>
    ...
    <li class="root <?php if(isset($menu['active']) and $menu['active']==1) echo " active";?> <?php if($menu['activemega']==1) echo " mega-root";?>"><a href="<?php echo $menu['url']; ?>"><?php echo html_entity_decode(html_entity_decode($title[$lang_id])) ?></a>
      <?php
			if($menu['activemega']==1)
				$MenuModel->list_submenu_mega($menu['id']);
			else
				$MenuModel->list_submenu($menu['id']);
		?>
    </li>
    <?php } ?>
  </ul>
...
<?php
if (file_exists(DIR_TEMPLATE . $configObject->get('config_template') . '/stylesheet/menu.css')) {
	$stylesheet = 'catalog/view/theme/'.$configObject->get('config_template').'/stylesheet/menu.css';
} else {
	$stylesheet = 'catalog/view/theme/default/stylesheet/menu.css';
}
?>
