          ...
          <div class="tab-content">
            <div class="tab-pane active" id="tab-general">
              ...
              <div class="form-group">
                <label class="col-sm-2 control-label" for="input-check-ip"><?php echo $entry_check_ip; ?></label>
                <div class="col-sm-10">
                  <select name="check_ip" id="input-check-ip" class="form-control">
                    <?php if ($check_ip) { ?>
...
$('#session button').on('click', function(e) {
	...
	if (confirm('<?php echo $text_confirm; ?>')) {
		...
		$.ajax({
			url: 'index.php?route=staff/api/deletesession&token=<?php echo $token; ?>&api_session_id=' + $(node).val(),
			type: 'post',
			dataType: 'json',
			beforeSend: function() {
				$(node).button('loading');
