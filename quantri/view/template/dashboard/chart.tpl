<div class="panel panel-default">
  ...
  <div class="panel-body">
    <div id="chart-manage" style="width: 100%; height: 260px;"></div>
  </div>
</div>
...
$('#range a').on('click', function(e) {
	...
	$.ajax({
		...
		success: function(json) {
			...
			$.plot('#chart-manage', [json['order'], json['user']], option);	
					
			$('#chart-manage').bind('plothover', function(event, pos, item) {
				$('.tooltip').remove();
				...
				if (item) {
					...
					$('#chart-manage').css('cursor', 'pointer');		
			  	} else {
					$('#chart-manage').css('cursor', 'auto');
				}
			});
		},
