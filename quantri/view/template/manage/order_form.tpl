      ...
      <div class="panel-body">
        <form class="form-horizontal">
          <ul id="order" class="nav nav-tabs nav-justified">
            <li class="disabled active"><a href="#tab-user" data-toggle="tab">1. <?php echo $tab_user; ?></a></li>
            <li class="disabled"><a href="#tab-cart" data-toggle="tab">2. <?php echo $tab_instrument; ?></a></li>
          ...
          <div class="tab-content">
            <div class="tab-pane active" id="tab-user">
              ...
              <div class="text-right">
                <button type="button" id="button-user" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><i class="fa fa-arrow-right"></i> <?php echo $button_continue; ?></button>
              </div>
            </div>
            <div class="tab-pane" id="tab-cart">
              <div class="table-responsive">
                <table class="table table-bordered">
                  ...
                  <tbody id="cart">
                    <?php if ($order_instruments || $order_vouchers) { ?>
                    <?php $instrument_row = 0; ?>
                    <?php foreach ($order_instruments as $order_instrument) { ?>
                    <tr>
                      <td class="text-left"><?php echo $order_instrument['name']; ?><br />
                        <input type="hidden" name="instrument[<?php echo $instrument_row; ?>][instrument_id]" value="<?php echo $order_instrument['instrument_id']; ?>" />
                        <?php foreach ($order_instrument['option'] as $option) { ?>
                        - <small><?php echo $option['name']; ?>: <?php echo $option['value']; ?></small><br />
                        <?php if ($option['type'] == 'select' || $option['type'] == 'radio' || $option['type'] == 'image') { ?>
                        <input type="hidden" name="instrument[<?php echo $instrument_row; ?>][option][<?php echo $option['instrument_option_id']; ?>]" value="<?php echo $option['instrument_option_value_id']; ?>" />
                        <?php } ?>
                        ...
                        <input type="hidden" name="instrument[<?php echo $instrument_row; ?>][option][<?php echo $option['instrument_option_id']; ?>]" value="<?php echo $option['value']; ?>" />
                        <?php } ?>
                        <?php } ?></td>
                      <td class="text-left"><?php echo $order_instrument['model']; ?></td>
                      <td class="text-right"><?php echo $order_instrument['quantity']; ?>
                        <input type="hidden" name="instrument[<?php echo $instrument_row; ?>][quantity]" value="<?php echo $order_instrument['quantity']; ?>" /></td>
              ...
              <ul class="nav nav-tabs nav-justified">
                <li class="active"><a href="#tab-instrument" data-toggle="tab"><?php echo $tab_instrument; ?></a></li>
                <li><a href="#tab-voucher" data-toggle="tab"><?php echo $tab_voucher; ?></a></li>
              </ul>
              <div class="tab-content">
                <div class="tab-pane active" id="tab-instrument">
                  <fieldset>
                    <legend><?php echo $text_instrument; ?></legend>
                    <div class="form-group">
                      <label class="col-sm-2 control-label" for="input-instrument"><?php echo $entry_instrument; ?></label>
                      <div class="col-sm-10">
                        <input type="text" name="instrument" value="" id="input-instrument" class="form-control" />
                  ...
                  <div class="text-right">
                    <button type="button" id="button-instrument-add" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><i class="fa fa-plus-circle"></i> <?php echo $button_instrument_add; ?></button>
                  </div>
                </div>
              ...
              <div class="row">
                <div class="col-sm-6 text-left">
                  <button type="button" onclick="$('a[href=\'#tab-user\']').tab('show');" class="btn btn-default"><i class="fa fa-arrow-left"></i> <?php echo $button_back; ?></button>
                </div>
...
$(document).delegate('#button-ip-add', 'click', function() {
	$.ajax({
		url: 'index.php?route=staff/api/addip&token=<?php echo $token; ?>&api_id=<?php echo $api_id; ?>',
		type: 'post',
...
// Add all instruments to the cart using the api
$('#button-refresh').on('click', function() {
	$.ajax({
		url: $('select[name=\'store\'] option:selected').val() + 'index.php?route=api/cart/instruments&token=' + token,
		dataType: 'json',
		crossDomain: true,
		success: function(json) {
			...
			if (json['instruments'].length) {
				for (i = 0; i < json['instruments'].length; i++) {
					instrument = json['instruments'][i];

					html += '<tr>';
					html += '  <td class="text-left">' + instrument['name'] + ' ' + (!instrument['stock'] ? '<span class="text-danger">***</span>' : '') + '<br />';
					html += '  <input type="hidden" name="instrument[' + i + '][instrument_id]" value="' + instrument['instrument_id'] + '" />';

					if (instrument['option']) {
						for (j = 0; j < instrument['option'].length; j++) {
							option = instrument['option'][j];

							html += '  - <small>' + option['name'] + ': ' + option['value'] + '</small><br />';

							if (option['type'] == 'select' || option['type'] == 'radio' || option['type'] == 'image') {
								html += '<input type="hidden" name="instrument[' + i + '][option][' + option['instrument_option_id'] + ']" value="' + option['instrument_option_value_id'] + '" />';
							}
							...
							if (option['type'] == 'text' || option['type'] == 'textarea' || option['type'] == 'file' || option['type'] == 'date' || option['type'] == 'datetime' || option['type'] == 'time') {
								html += '<input type="hidden" name="instrument[' + i + '][option][' + option['instrument_option_id'] + ']" value="' + option['value'] + '" />';
							}
						}
					}

					html += '</td>';
					html += '  <td class="text-left">' + instrument['model'] + '</td>';
					html += '  <td class="text-right"><div class="input-group btn-block" style="max-width: 200px;"><input type="text" name="instrument[' + i + '][quantity]" value="' + instrument['quantity'] + '" class="form-control" /><span class="input-group-btn"><button type="button" data-toggle="tooltip" title="<?php echo $button_refresh; ?>" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-primary"><i class="fa fa-refresh"></i></button></span></div></td>';
                    html += '  <td class="text-right">' + instrument['price'] + '</td>';
					html += '  <td class="text-right">' + instrument['total'] + '</td>';
					html += '  <td class="text-center" style="width: 3px;"><button type="button" value="' + instrument['cart_id'] + '" data-toggle="tooltip" title="<?php echo $button_remove; ?>" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-danger"><i class="fa fa-minus-circle"></i></button></td>';
					html += '</tr>';
			...
			if (!json['instruments'].length && !json['vouchers'].length) {
				html += '<tr>';
			...
			if (json['instruments'].length) {
				for (i = 0; i < json['instruments'].length; i++) {
					...
					html += '  </td>';
					html += '  <td class="text-left">' + instrument['model'] + '</td>';
					html += '  <td class="text-right">' + instrument['quantity'] + '</td>';
					html += '  <td class="text-right">' + instrument['price'] + '</td>';
					html += '  <td class="text-right">' + instrument['total'] + '</td>';
					html += '</tr>';
				}
			...
			if (!json['totals'].length && !json['instruments'].length && !json['vouchers'].length) {
				html += '<tr>';
...
$('input[name=\'user\']').autocomplete({
	...
	'select': function(item) {
		...
		$('#tab-user input[type=\'text\'], #tab-user textarea').not('#tab-user input[name=\'user\'], #tab-user input[name=\'user_id\']').val('');
		$('#tab-user select option').removeAttr('selected');
		$('#tab-user input[type=\'checkbox\'], #tab-user input[type=\'radio\']').removeAttr('checked');

		$('#tab-user input[name=\'user\']').val(item['label']);
		$('#tab-user input[name=\'user_id\']').val(item['value']);
		$('#tab-user select[name=\'user_group_id\']').val(item['user_group_id']);
		$('#tab-user input[name=\'firstname\']').val(item['firstname']);
		$('#tab-user input[name=\'lastname\']').val(item['lastname']);
		$('#tab-user input[name=\'email\']').val(item['email']);
		$('#tab-user input[name=\'telephone\']').val(item['telephone']);
		$('#tab-user input[name=\'fax\']').val(item['fax']);

		for (i in item.custom_field) {
			$('#tab-user select[name=\'custom_field[' + i + ']\']').val(item.custom_field[i]);
			$('#tab-user textarea[name=\'custom_field[' + i + ']\']').val(item.custom_field[i]);
			$('#tab-user input[name^=\'custom_field[' + i + ']\'][type=\'text\']').val(item.custom_field[i]);
			$('#tab-user input[name^=\'custom_field[' + i + ']\'][type=\'hidden\']').val(item.custom_field[i]);
			$('#tab-user input[name^=\'custom_field[' + i + ']\'][type=\'radio\'][value=\'' + item.custom_field[i] + '\']').prop('checked', true);

			if (item.custom_field[i] instanceof Array) {
				for (j = 0; j < item.custom_field[i].length; j++) {
					$('#tab-user input[name^=\'custom_field[' + i + ']\'][type=\'checkbox\'][value=\'' + item.custom_field[i][j] + '\']').prop('checked', true);
				}
			}
		}
...
$('select[name=\'user_group_id\']').on('change', function() {
	$.ajax({
		url: 'index.php?route=user/user/customfield&token=<?php echo $token; ?>&user_group_id=' + this.value,
		dataType: 'json',
		success: function(json) {
			$('.custom-field').hide();
...
$('#button-user').on('click', function() {
	$.ajax({
		url: $('select[name=\'store\'] option:selected').val() + 'index.php?route=api/user&token=' + token,
		type: 'post',
		data: $('#tab-user input[type=\'text\'], #tab-user input[type=\'hidden\'], #tab-user input[type=\'radio\']:checked, #tab-user input[type=\'checkbox\']:checked, #tab-user select, #tab-user textarea'),
		dataType: 'json',
		crossDomain: true,
		beforeSend: function() {
			$('#button-user').button('loading');
		},
		complete: function() {
			 $('#button-user').button('reset');
		},
		success: function(json) {
			$('.alert, .text-danger').remove();
			$('.form-group').removeClass('has-error');
			...
			if (json['error']) {
                ...
                // Refresh instruments, vouchers and totals
                var request_1 = $.ajax({
                    url: $('select[name=\'store\'] option:selected').val() + 'index.php?route=api/cart/add&token=' + token,
                    type: 'post',
                    data: $('#cart input[name^=\'instrument\'][type=\'text\'], #cart input[name^=\'instrument\'][type=\'hidden\'], #cart input[name^=\'instrument\'][type=\'radio\']:checked, #cart input[name^=\'instrument\'][type=\'checkbox\']:checked, #cart select[name^=\'instrument\'], #cart textarea[name^=\'instrument\']'),
                    dataType: 'json',
                    crossDomain: true,
                    beforeSend: function() {
                        $('#button-instrument-add').button('loading');
                    },
                    complete: function() {
                        $('#button-instrument-add').button('reset');
                    },
                    success: function(json) {
                        $('.alert, .text-danger').remove();
                        $('.form-group').removeClass('has-error');
...
$('#tab-instrument input[name=\'instrument\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
	...
	'select': function(item) {
		$('#tab-instrument input[name=\'instrument\']').val(item['label']);
		$('#tab-instrument input[name=\'instrument_id\']').val(item['value']);
		...
		if (item['option'] != '') {
			...
			for (i = 0; i < item['option'].length; i++) {
				...
				if (option['type'] == 'select') {
					...
					html += '  <label class="col-sm-2 control-label" for="input-option' + option['instrument_option_id'] + '">' + option['name'] + '</label>';
					html += '  <div class="col-sm-10">';
					html += '    <select name="option[' + option['instrument_option_id'] + ']" id="input-option' + option['instrument_option_id'] + '" class="form-control">';
					html += '      <option value=""><?php echo $text_select; ?></option>';

					for (j = 0; j < option['instrument_option_value'].length; j++) {
						option_value = option['instrument_option_value'][j];

						html += '<option value="' + option_value['instrument_option_value_id'] + '">' + option_value['name'];
				...
				if (option['type'] == 'checkbox') {
					...
					html += '  <div class="col-sm-10">';
					html += '    <div id="input-option' + option['instrument_option_id'] + '">';
					...
					for (j = 0; j < option['instrument_option_value'].length; j++) {
						...
						html += '  <label><input type="checkbox" name="option[' + option['instrument_option_id'] + '][]" value="' + option_value['instrument_option_value_id'] + '" /> ' + option_value['name'];
				...
				if (option['type'] == 'text') {
					html += '<div class="form-group' + (option['required'] ? ' required' : '') + '">';
					html += '  <label class="col-sm-2 control-label" for="input-option' + option['instrument_option_id'] + '">' + option['name'] + '</label>';
					html += '  <div class="col-sm-10"><input type="text" name="option[' + option['instrument_option_id'] + ']" value="' + option['value'] + '" id="input-option' + option['instrument_option_id'] + '" class="form-control" /></div>';
					html += '</div>';
				}
				...
				if (option['type'] == 'textarea') {
					html += '<div class="form-group' + (option['required'] ? ' required' : '') + '">';
					html += '  <label class="col-sm-2 control-label" for="input-option' + option['instrument_option_id'] + '">' + option['name'] + '</label>';
					html += '  <div class="col-sm-10"><textarea name="option[' + option['instrument_option_id'] + ']" rows="5" id="input-option' + option['instrument_option_id'] + '" class="form-control">' + option['value'] + '</textarea></div>';
					html += '</div>';
				}
				...
				if (option['type'] == 'file') {
					...
					html += '  <label class="col-sm-2 control-label">' + option['name'] + '</label>';
					html += '  <div class="col-sm-10">';
					html += '    <button type="button" id="button-upload' + option['instrument_option_id'] + '" data-loading-text="<?php echo $text_loading; ?>" class="btn btn-default"><i class="fa fa-upload"></i> <?php echo $button_upload; ?></button>';
					html += '    <input type="hidden" name="option[' + option['instrument_option_id'] + ']" value="' + option['value'] + '" id="input-option' + option['instrument_option_id'] + '" />';
					html += '  </div>';
					html += '</div>';
				}
				...
				if (option['type'] == 'date') {
					html += '<div class="form-group' + (option['required'] ? ' required' : '') + '">';
					html += '  <label class="col-sm-2 control-label" for="input-option' + option['instrument_option_id'] + '">' + option['name'] + '</label>';
					html += '  <div class="col-sm-3"><div class="input-group date"><input type="text" name="option[' + option['instrument_option_id'] + ']" value="' + option['value'] + '" placeholder="' + option['name'] + '" data-date-format="YYYY-MM-DD" id="input-option' + option['instrument_option_id'] + '" class="form-control" /><span class="input-group-btn"><button type="button" class="btn btn-default"><i class="fa fa-calendar"></i></button></span></div></div>';
					html += '</div>';
				}
				...
				if (option['type'] == 'datetime') {
					html += '<div class="form-group' + (option['required'] ? ' required' : '') + '">';
					html += '  <label class="col-sm-2 control-label" for="input-option' + option['instrument_option_id'] + '">' + option['name'] + '</label>';
					html += '  <div class="col-sm-3"><div class="input-group datetime"><input type="text" name="option[' + option['instrument_option_id'] + ']" value="' + option['value'] + '" placeholder="' + option['name'] + '" data-date-format="YYYY-MM-DD HH:mm" id="input-option' + option['instrument_option_id'] + '" class="form-control" /><span class="input-group-btn"><button type="button" class="btn btn-default"><i class="fa fa-calendar"></i></button></span></div></div>';
					html += '</div>';
				}
				...
				if (option['type'] == 'time') {
					html += '<div class="form-group' + (option['required'] ? ' required' : '') + '">';
					html += '  <label class="col-sm-2 control-label" for="input-option' + option['instrument_option_id'] + '">' + option['name'] + '</label>';
					html += '  <div class="col-sm-3"><div class="input-group time"><input type="text" name="option[' + option['instrument_option_id'] + ']" value="' + option['value'] + '" placeholder="' + option['name'] + '" data-date-format="HH:mm" id="input-option' + option['instrument_option_id'] + '" class="form-control" /><span class="input-group-btn"><button type="button" class="btn btn-default"><i class="fa fa-calendar"></i></button></span></div></div>';
					html += '</div>';
				}
			}
...
$('#button-instrument-add').on('click', function() {
	$.ajax({
		url: $('select[name=\'store\'] option:selected').val() + 'index.php?route=api/cart/add&token=' + token,
		type: 'post',
		data: $('#tab-instrument input[name=\'instrument_id\'], #tab-instrument input[name=\'quantity\'], #tab-instrument input[name^=\'option\'][type=\'text\'], #tab-instrument input[name^=\'option\'][type=\'hidden\'], #tab-instrument input[name^=\'option\'][type=\'radio\']:checked, #tab-instrument input[name^=\'option\'][type=\'checkbox\']:checked, #tab-instrument select[name^=\'option\'], #tab-instrument textarea[name^=\'option\']'),
		dataType: 'json',
		crossDomain: true,
		beforeSend: function() {
			$('#button-instrument-add').button('loading');
...
$('select[name=\'payment_address\']').on('change', function() {
	$.ajax({
		url: 'index.php?route=user/user/address&token=<?php echo $token; ?>&address_id=' + this.value,
		dataType: 'json',
		beforeSend: function() {
...
$('#tab-user .form-group[data-sort]').detach().each(function() {
	if ($(this).attr('data-sort') >= 0 && $(this).attr('data-sort') <= $('#tab-user .form-group').length) {
		$('#tab-user .form-group').eq($(this).attr('data-sort')).before(this);
	}

	if ($(this).attr('data-sort') > $('#tab-user .form-group').length) {
		$('#tab-user .form-group:last').after(this);
	}

	if ($(this).attr('data-sort') < -$('#tab-user .form-group').length) {
		$('#tab-user .form-group:first').before(this);
	}
});
