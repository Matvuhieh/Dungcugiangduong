	...
	public function index() {
		...
		foreach ($results as $result) {
			...
			$replace = array(
				...
				$this->url->link('manage/order/info', 'token=' . $this->session->data['token'] . '&order_id=', 'SSL'),
				$this->url->link('marketing/affiliate/edit', 'token=' . $this->session->data['token'] . '&affiliate_id=', 'SSL'),
				$this->url->link('manage/return/edit', 'token=' . $this->session->data['token'] . '&return_id=', 'SSL')
			);
