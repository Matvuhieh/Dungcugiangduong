<?php
class ControllerCommonFileOrderr extends Controller {
	...
	public function upload() {
		...
		if (!$this->staff->hasPermission('modify', 'common/filemanager')) {
		...
		if (!empty($this->request->post['watermark'])) {
		...
		foreach($this->request->files['file']['name'] as $key => $value) {
			if (!$json) {
				if (!empty($this->request->files['file']['name'][$key]) && is_file($this->request->files['file']['tmp_name'][$key])) {
					// Sanitize the filename
					$filename[$key] = $this->str_alias(basename(html_entity_decode($this->request->files['file']['name'][$key], ENT_QUOTES, 'UTF-8')));
					
					
					// Validate the filename length
					if ((utf8_strlen($filename[$key]) < 3) || (utf8_strlen($filename[$key]) > 255)) {
					...
					if (!in_array(utf8_strtolower(utf8_substr(strrchr($filename[$key], '.'), 1)), $allowed)) {
		...
		foreach($this->request->files['file']['name'] as $key => $value) {
			if (!$json) {
				$dimension = getimagesize($this->request->files['file']['tmp_name'][$key]);
				if((int)$dimension[0] > 4000 || (int)$dimension[1] > 4000){
					$json['error'] = $this->language->get('error_dimension');
				}
			}
		}
		...
		if (!$json) {
			foreach($this->request->files['file']['name'] as $key => $value) {
				move_uploaded_file($this->request->files['file']['tmp_name'][$key], $directory . '/' . $filename[$key]);
				
				if($watermark != 'none' && in_array(utf8_strtolower(utf8_substr(strrchr($filename[$key], '.'), 1)), $allowed)) {
					$mark = new Image(DIR_IMAGE . $this->config->get('config_watermark'));
					
					$image = new Image($directory . '/' . $filename[$key]);
					$image->watermark($mark, $watermark);
					$image->save($directory . '/' . $filename[$key]);
		...
		if(!$json) $json['success'] = $this->language->get('text_uploaded');
	...
	public function folder() {
		...
		if (!$json) {
			...
			$folder = $this->str_alias(str_replace(array('../', '..\\', '..'), '', basename(html_entity_decode($this->request->post['folder'], ENT_QUOTES, 'UTF-8'))));
	...
	public function str_alias($string = ''){
