...
class ControllerToolExportImport extends Controller { 
	...
	protected function getForm() {
		...
		$data['text_export_type_instrument'] = ($data['exist_filter']) ? $this->language->get('text_export_type_instrument') : $this->language->get('text_export_type_instrument_old');
		...
		$data['text_export_type_user'] = $this->language->get('text_export_type_user');
		...
		$min_instrument_id = $this->model_tool_export_import->getMinInstrumentId();
		$max_instrument_id = $this->model_tool_export_import->getMaxInstrumentId();
		$count_instrument = $this->model_tool_export_import->getCountInstrument();
		...
		$min_user_id = $this->model_tool_export_import->getMinUserId();
		$max_user_id = $this->model_tool_export_import->getMaxUserId();
		$count_user = $this->model_tool_export_import->getCountUser();
		
		$data['min_instrument_id'] = $min_instrument_id;
		$data['max_instrument_id'] = $max_instrument_id;
		$data['count_instrument'] = $count_instrument;
		$data['min_category_id'] = $min_category_id;
		$data['max_category_id'] = $max_category_id;
		$data['count_category'] = $count_category;
		$data['min_user_id'] = $min_user_id;
	...
	protected function validateDownloadForm() {
		if (!$this->staff->hasPermission('access', 'tool/export_import')) {
	...
	protected function validateUploadForm() {
		if (!$this->staff->hasPermission('modify', 'tool/export_import')) {
			$this->error['warning'] = $this->language->get('error_permission');
