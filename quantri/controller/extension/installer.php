...
class ControllerExtensionInstaller extends Controller {
	...
	public function upload() {
		...
		if (!$this->staff->hasPermission('modify', 'extension/installer')) {
	...
	public function ftp() {
		...
		if (!$json) {
			...
			if ($connection) {
				$login = ftp_login($connection, $this->config->get('config_ftp_staffname'), $this->config->get('config_ftp_password'));
				...
				if ($login) {
					...
					$json['error'] = sprintf($this->language->get('error_ftp_login'), $this->config->get('config_ftp_staffname'));
				}
