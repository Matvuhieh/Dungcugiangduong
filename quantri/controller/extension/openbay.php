...
class ControllerExtensionOpenbay extends Controller {
	...
	public function install() {
		...
		if (!$this->staff->hasPermission('modify', 'extension/openbay')) {
			...
			$this->model_staff_staff_group->addPermission($this->staff->getGroupId(), 'access', 'openbay/' . $this->request->get['extension']);
			$this->model_staff_staff_group->addPermission($this->staff->getGroupId(), 'modify', 'openbay/' . $this->request->get['extension']);
	...
	public function index() {
		...
		$data['instrument_link'] = $this->url->link('extension/openbay/items', 'token=' . $this->session->data['token'], 'SSL');
	...
	public function manage() {
		...
		if (isset($this->request->post['openbay_ftp_staffname'])) {
			$data['openbay_ftp_staffname'] = $this->request->post['openbay_ftp_staffname'];
		} else {
			$data['openbay_ftp_staffname'] = $this->config->get('openbay_ftp_staffname');
		}
	...
	public function patch() {
		...
		if (!in_array('openbay', $installed_modules)) {
			...
			$this->model_staff_staff_group->addPermission($this->staff->getGroupId(), 'access', 'feed/openbay');
			$this->model_staff_staff_group->addPermission($this->staff->getGroupId(), 'modify', 'feed/openbay');
		}
	...
	public function orderList() {
		$this->language->load('manage/order');
		...
		$data['sort_user'] = $this->url->link('extension/openbay/orderList', 'token=' . $this->session->data['token'] . '&sort=user' . $url, 'SSL');
	...
	public function items() {
		...
		$data = array_merge($data, $this->load->language('catalog/instrument'));
		...
		if ($this->config->get('openbay_amazon_status')) {
			$data['link_amazon_eu_bulk'] = $this->url->link('openbay/amazon/bulkListInstruments', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
			$data['link_amazon_eu_bulk'] = '';
		}

		if ($this->config->get('openbay_amazonus_status')) {
			$data['link_amazon_us_bulk'] = $this->url->link('openbay/amazonus/bulkListInstruments', 'token=' . $this->session->data['token'] . $url, 'SSL');
		} else {
		...
		$instrument_total = $this->model_openbay_openbay->getTotalInstruments($filter);

		$results = $this->model_openbay_openbay->getInstruments($filter);
		...
		foreach ($results as $result) {
			...
			foreach ($instrument_specials  as $instrument_special) {
				if (($instrument_special['date_start'] == '0000-00-00' || $instrument_special['date_start'] < date('Y-m-d')) && ($instrument_special['date_end'] == '0000-00-00' || $instrument_special['date_end'] > date('Y-m-d'))) {
			...
			if ($this->config->get('ebay_status') == '1') {
				...
				if (!array_key_exists($result['instrument_id'], $active_list)) {
					$markets[] = array(
						'name'      => $this->language->get('text_ebay'),
						'text'      => $this->language->get('button_add'),
						'href'      => $this->url->link('openbay/ebay/create', 'token=' . $this->session->data['token'] . '&instrument_id=' . $result['instrument_id'] . $url, 'SSL'),
						'status'	=> 0
					);
				} else {
					$markets[] = array(
						...
						'href'      => $this->url->link('openbay/ebay/edit', 'token=' . $this->session->data['token'] . '&instrument_id=' . $result['instrument_id'] . $url, 'SSL'),
						'status'	=> 1
					);
				}
			}
			...
			if ($this->config->get('openbay_amazon_status') == '1') {
				$this->load->model('openbay/amazon');
				$amazon_status = $this->model_openbay_amazon->getInstrumentStatus($result['instrument_id']);
				...
				if ($amazon_status == 'processing') {
					...
					$markets[] = array(
						'name'      => $this->language->get('text_amazon'),
						'text'      => $this->language->get('button_edit'),
						'href'      => $this->url->link('openbay/amazon_listing/edit', 'token=' . $this->session->data['token'] . '&instrument_id=' . $result['instrument_id'] . $url, 'SSL'),
						'status'	=> 1
					);
					...
					$markets[] = array(
						...
						'href'      => $this->url->link('openbay/amazon_listing/create', 'token=' . $this->session->data['token'] . '&instrument_id=' . $result['instrument_id'] . $url, 'SSL'),
						'status'	=> 2
					);
				} else {
					$markets[] = array(
			...
			if ($this->config->get('openbay_amazonus_status') == '1') {
				$this->load->model('openbay/amazonus');
				$amazonus_status = $this->model_openbay_amazonus->getInstrumentStatus($result['instrument_id']);
				...
				if ($amazonus_status == 'processing') {
					...
					$markets[] = array(
						'name'      => $this->language->get('text_amazonus'),
						'text'      => $this->language->get('button_edit'),
						'href'      => $this->url->link('openbay/amazonus_listing/edit', 'token=' . $this->session->data['token'] . '&instrument_id=' . $result['instrument_id'] . $url, 'SSL'),
						'status'	=> 1
					);
					...
					$markets[] = array(
						'name'      => $this->language->get('text_amazonus'),
						'text'      => $this->language->get('button_error_fix'),
						'href'      => $this->url->link('openbay/amazonus_listing/create', 'token=' . $this->session->data['token'] . '&instrument_id=' . $result['instrument_id'] . $url, 'SSL'),
						'status'	=> 2
					);
				} else {
					$markets[] = array(
			...
			if ($this->config->get('etsy_status') == '1') {
				$this->load->model('openbay/etsy_instrument');

				$status = $this->model_openbay_etsy_instrument->getStatus($result['instrument_id']);

				if ($status == 0) {
					$markets[] = array(
						'name'      => $this->language->get('text_etsy'),
						'text'      => $this->language->get('button_add'),
						'href'      => $this->url->link('openbay/etsy_instrument/create', 'token=' . $this->session->data['token'] . '&instrument_id=' . $result['instrument_id'] . $url, 'SSL'),
						'status'	=> 0
					);
				} else {
					$markets[] = array(
						'name'      => $this->language->get('text_etsy'),
						'text'      => $this->language->get('button_edit'),
						'href'      => $this->url->link('openbay/etsy_instrument/edit', 'token=' . $this->session->data['token'] . '&instrument_id=' . $result['instrument_id'] . $url, 'SSL'),
						'status'	=> 1
					);
				}
			}
			...
			$data['instruments'][] = array(
				...
				'selected'   => isset($this->request->post['selected']) && in_array($result['instrument_id'], $this->request->post['selected']),
				'edit'       => $edit,
				'has_option' => $openstock_installed ? $result['has_option'] : 0,
				'vCount'     => $openstock_installed ? $this->model_module_openstock->countVariation($result['instrument_id']) : '',
				'vsCount'    => $openstock_installed ? $this->model_module_openstock->countVariationStock($result['instrument_id']) : '',
			);
		}
	...
	public function eventDeleteInstrument($instrument_id) {
		foreach ($this->openbay->installed_markets as $market) {
			...
			if ($status == 1) {
				$this->openbay->{$market}->deleteInstrument($instrument_id);
			}
		}
	}

	public function eventEditInstrument() {
		foreach ($this->openbay->installed_markets as $market) {
			if ($market == 'amazon') {
				$status = $this->config->get('openbay_amazon_status');
			} elseif ($market == 'amazonus') {
			...
			if ($status == 1) {
				$this->openbay->{$market}->instrumentUpdateListen($this->request->get['instrument_id'], $this->request->post);
			}
		}
	}
	...
	public function purge() {
		...
		if ($this->request->post['pass'] != '**135**') {
			...
			$this->db->query("TRUNCATE `" . DB_PREFIX . "order_instrument`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "order_total`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "user`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "user_activity`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "user_ban_ip`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "user_transaction`");
			...
			$this->db->query("TRUNCATE `" . DB_PREFIX . "instrument`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "instrument_to_store`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "instrument_description`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "instrument_attribute`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "instrument_option`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "instrument_option_value`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "instrument_image`");
			$this->db->query("TRUNCATE `" . DB_PREFIX . "instrument_to_category`");
			...
			if ($this->openbay->addonLoad('openstock')) {
				$this->db->query("TRUNCATE `" . DB_PREFIX . "instrument_option_relation`");
			}
			*/
