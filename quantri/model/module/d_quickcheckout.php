...
class ModelModuleDQuickborrow extends Model {
	...
	public function getThemes(){
		$dir = DIR_CATALOG.'view/theme/default/stylesheet/d_quickborrow/theme';
		$files = scandir($dir);
		$result = array();
	...
	*/
    ...
    public function getSettings($store_id, $rating = false){
    	...
    	foreach ($results as $key => $result) {
			...
			if($rating){
				$query = $this->db->query("SELECT AVG((date_modified - date_added)) as average_borrow_time, AVG(rating) as average_rating FROM " . DB_PREFIX . "dqc_statistic WHERE setting_id = '" . (int)$result['setting_id'] . "' LIMIT 50");
				$results[$key]['average_borrow_time'] = $query->row['average_borrow_time'];
    ...
    public function getStatistics($setting_id){
    	$query = $this->db->query("SELECT *, (s.date_modified - s.date_added) as borrow_time FROM `" . DB_PREFIX . "dqc_statistic` s 
	...
	public function getCustomFieldsByLocation($location){
		if(VERSION >= '2.1.0.1'){
			...
			$results = $this->model_user_custom_field->getCustomFields();	
		}else{
			$this->load->model('manage/custom_field');
			$results = $this->model_manage_custom_field->getCustomFields();	
		}
