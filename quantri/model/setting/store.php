...
class ModelSettingStore extends Model {
	...
	public function getTotalStoresByUserGroupId($user_group_id) {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "setting WHERE `key` = 'config_user_group_id' AND `value` = '" . (int)$user_group_id . "' AND store_id != '0'");
	...
	public function getTotalStoresByInformationId($information_id) {
		...
		$borrow_query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "setting WHERE `key` = 'config_borrow_id' AND `value` = '" . (int)$information_id . "' AND store_id != '0'");

		return ($account_query->row['total'] + $borrow_query->row['total']);
	}
