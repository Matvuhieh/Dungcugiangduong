...
class ModelZOrderrTree extends Model {
    protected $base = null;
    ...
    public function __construct($registry) {
        ...
        if(self::$base_path) {
            $this->base = self::$base_path;
        }
    }

    public function initBase($base) {
        $this->base = $this->real($base);
        if (!$this->base) {
            throw new Exception('Base directory does not exist');
        }
        self::$base_path = $this->base;
    }
    ...
    public function search($id, $search, $with_root = false) {
        $lst = glob($this->base . DIRECTORY_SEPARATOR  . '*.*');
       ...
       // print_r($lst);
        ...
        $res = array();
        foreach ($lst as $item) {
            $exp = explode(DIRECTORY_SEPARATOR, $item);
            if(!count($exp)) {
                continue;
            }
            $tmp = preg_match('([^ a-zа-я-_0-9.]+)ui', $exp[count($exp) - 1]);
            ...
            if (is_dir($item)) {
                $res[] = array('text' => $item, 'children' => true, 'id' => $item, 'icon' => 'folder');
            } else {
                $res[] = array('text' => $item, 'children' => false, 'id' => $item, 'type' => 'file', 'icon' => 'file file-' . substr($item, strrpos($item, '.') + 1));
            }
        }
        ...
        if (is_file($dir)) {
            ...
            switch ($ext) {
                ...
                case 'bmp':
                    ...
                    $dat['content'] = 'data:' . $type . ';base64,' . base64_encode(file_get_contents($dir));
                    break;
                default:
    ...
    public function saveFileContent($id, $content) {
        $filepath = $this->path($id);
        if (is_file($filepath) && is_writable($filepath)) {

            //$content = iconv(mb_detect_encoding(file_get_contents($file), mb_detect_order(), true), "UTF-8", $content);
