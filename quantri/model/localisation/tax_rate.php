...
class ModelLocalisationTaxRate extends Model {
	public function addTaxRate($data) {
		...
		if (isset($data['tax_rate_user_group'])) {
			foreach ($data['tax_rate_user_group'] as $user_group_id) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "tax_rate_to_user_group SET tax_rate_id = '" . (int)$tax_rate_id . "', user_group_id = '" . (int)$user_group_id . "'");
			}
		}
	}
	...
	public function editTaxRate($tax_rate_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "tax_rate_to_user_group WHERE tax_rate_id = '" . (int)$tax_rate_id . "'");
	...
	public function getTaxRateUserGroups($tax_rate_id) {
		$tax_user_group_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tax_rate_to_user_group WHERE tax_rate_id = '" . (int)$tax_rate_id . "'");

		foreach ($query->rows as $result) {
			$tax_user_group_data[] = $result['user_group_id'];
		}

		return $tax_user_group_data;
	}
