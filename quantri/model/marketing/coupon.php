...
class ModelMarketingCoupon extends Model {
	...
	public function getCouponInstruments($coupon_id) {
		$coupon_instrument_data = array();

		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "coupon_instrument WHERE coupon_id = '" . (int)$coupon_id . "'");

		foreach ($query->rows as $result) {
			$coupon_instrument_data[] = $result['instrument_id'];
		}

		return $coupon_instrument_data;
	}
	...
	public function getCouponHistories($coupon_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT ch.order_id, CONCAT(c.firstname, ' ', c.lastname) AS user, ch.amount, ch.date_added FROM " . DB_PREFIX . "coupon_history ch LEFT JOIN " . DB_PREFIX . "user c ON (ch.user_id = c.user_id) WHERE ch.coupon_id = '" . (int)$coupon_id . "' ORDER BY ch.date_added ASC LIMIT " . (int)$start . "," . (int)$limit);
