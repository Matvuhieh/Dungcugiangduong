...
class ModelUserCustomField extends Model {
		...
		if (isset($data['custom_field_user_group'])) {
			foreach ($data['custom_field_user_group'] as $custom_field_user_group) {
				if (isset($custom_field_user_group['user_group_id'])) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "custom_field_user_group SET custom_field_id = '" . (int)$custom_field_id . "', user_group_id = '" . (int)$custom_field_user_group['user_group_id'] . "', required = '" . (int)(isset($custom_field_user_group['required']) ? 1 : 0) . "'");
				}
			}
		}
	...
	public function getCustomFieldUserGroups($custom_field_id) {
