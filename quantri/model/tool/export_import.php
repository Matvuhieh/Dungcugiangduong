...
class ModelToolExportImport extends Model {
	...
	protected function getAvailableInstrumentIds(&$data) {
		$available_instrument_ids = array();
		...
		for ($i=1; $i<$k; $i+=1) {
			...
			$available_instrument_ids[$instrument_id] = $instrument_id;
		}
		return $available_instrument_ids;
	}
	...
	protected function getUserGroupIds() {
		$sql = "SHOW TABLES LIKE \"".DB_PREFIX."user_group_description\"";
		$query = $this->db->query( $sql );
		if ($query->num_rows) {
			$language_id = $this->getDefaultLanguageId();
			$sql  = "SELECT `user_group_id`, `name` FROM `".DB_PREFIX."user_group_description` ";
			$sql .= "WHERE language_id=$language_id ";
			$sql .= "ORDER BY `user_group_id` ASC";
			$query = $this->db->query( $sql );
		} else {
		...
		foreach ($query->rows as $row) {
			...
			$name = $row['name'];
			$user_group_ids[$name] = $user_group_id;
		}
	...
	protected function getInstrumentViewCounts() {
		$query = $this->db->query( "SELECT instrument_id, viewed FROM `".DB_PREFIX."instrument`" );
		$view_counts = array();
		foreach ($query->rows as $row) {
			$instrument_id = $row['instrument_id'];
			$viewed = $row['viewed'];
			$view_counts[$instrument_id] = $viewed;
		}
		return $view_counts;
	}


	protected function getInstrumentUrlAliasIds() {
		$sql  = "SELECT url_alias_id, SUBSTRING( query, CHAR_LENGTH('instrument_id=')+1 ) AS instrument_id ";
		$sql .= "FROM `".DB_PREFIX."url_alias` ";
		$sql .= "WHERE query LIKE 'instrument_id=%'";
		$query = $this->db->query( $sql );
		$url_alias_ids = array();
		foreach ($query->rows as $row) {
			$url_alias_id = $row['url_alias_id'];
			$instrument_id = $row['instrument_id'];
			$url_alias_ids[$instrument_id] = $url_alias_id;
		}
		return $url_alias_ids;
	}


	protected function storeInstrumentIntoDatabase( &$instrument, &$languages, &$instrument_fields, $exist_table_instrument_tag, $exist_meta_title, &$layout_ids, &$available_store_ids, &$manufacturers, &$weight_class_ids, &$length_class_ids, &$url_alias_ids ) {
		// extract the instrument details
		$instrument_id = $instrument['instrument_id'];
		$names = $instrument['names'];
		$categories = $instrument['categories'];
		$quantity = $instrument['quantity'];
		$model = $this->db->escape($instrument['model']);
		$manufacturer_name = $instrument['manufacturer_name'];
		$image = $this->db->escape($instrument['image']);
		$shipping = $instrument['shipping'];
		$shipping = ((strtoupper($shipping)=="YES") || (strtoupper($shipping)=="Y") || (strtoupper($shipping)=="TRUE")) ? 1 : 0;
		$price = trim($instrument['price']);
		$points = $instrument['points'];
		$date_added = $instrument['date_added'];
		$date_modified = $instrument['date_modified'];
		$date_available = $instrument['date_available'];
		$weight = ($instrument['weight']=="") ? 0 : $instrument['weight'];
		$weight_unit = $instrument['weight_unit'];
		$weight_class_id = (isset($weight_class_ids[$weight_unit])) ? $weight_class_ids[$weight_unit] : 0;
		$status = $instrument['status'];
		$status = ((strtoupper($status)=="TRUE") || (strtoupper($status)=="YES") || (strtoupper($status)=="ENABLED")) ? 1 : 0;
		$tax_class_id = $instrument['tax_class_id'];
		$viewed = $instrument['viewed'];
		$descriptions = $instrument['descriptions'];
		$stock_status_id = $instrument['stock_status_id'];
		if ($exist_meta_title) {
			$meta_titles = $instrument['meta_titles'];
		}
		$meta_descriptions = $instrument['meta_descriptions'];
		$length = $instrument['length'];
		$width = $instrument['width'];
		$height = $instrument['height'];
		$keyword = $this->db->escape($instrument['seo_keyword']);
		$length_unit = $instrument['measurement_unit'];
		$length_class_id = (isset($length_class_ids[$length_unit])) ? $length_class_ids[$length_unit] : 0;
		$sku = $this->db->escape($instrument['sku']);
		$upc = $this->db->escape($instrument['upc']);
		if (in_array('ean',$instrument_fields)) {
			$ean = $this->db->escape($instrument['ean']);
		}
		if (in_array('jan',$instrument_fields)) {
			$jan = $this->db->escape($instrument['jan']);
		}
		if (in_array('isbn',$instrument_fields)) {
			$isbn = $this->db->escape($instrument['isbn']);
		}
		if (in_array('mpn',$instrument_fields)) {
			$mpn = $this->db->escape($instrument['mpn']);
		}
		$location = $this->db->escape($instrument['location']);
		$store_ids = $instrument['store_ids'];
		$layout = $instrument['layout'];
		$related_ids = $instrument['related_ids'];
		...
		// generate and execute SQL for inserting the instrument
		$sql  = "INSERT INTO `".DB_PREFIX."instrument` (`instrument_id`,`quantity`,`sku`,`upc`,";
		$sql .= in_array('ean',$instrument_fields) ? "`ean`," : "";
		$sql .= in_array('jan',$instrument_fields) ? "`jan`," : "";
		$sql .= in_array('isbn',$instrument_fields) ? "`isbn`," : "";
		$sql .= in_array('mpn',$instrument_fields) ? "`mpn`," : "";
		...
		$sql .= "($instrument_id,$quantity,'$sku','$upc',";
		$sql .= in_array('ean',$instrument_fields) ? "'$ean'," : "";
		$sql .= in_array('jan',$instrument_fields) ? "'$jan'," : "";
		$sql .= in_array('isbn',$instrument_fields) ? "'$isbn'," : "";
		$sql .= in_array('mpn',$instrument_fields) ? "'$mpn'," : "";
		...
		foreach ($languages as $language) {
			...
			if ($exist_table_instrument_tag) {
				if ($exist_meta_title) {
					$sql  = "INSERT INTO `".DB_PREFIX."instrument_description` (`instrument_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_keyword`) VALUES ";
					$sql .= "( $instrument_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_keyword' );";
				} else {
					$sql  = "INSERT INTO `".DB_PREFIX."instrument_description` (`instrument_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`) VALUES ";
					$sql .= "( $instrument_id, $language_id, '$name', '$description', '$meta_description', '$meta_keyword' );";
				}
				$this->db->query( $sql );
				$sql  = "INSERT INTO `".DB_PREFIX."instrument_tag` (`instrument_id`,`language_id`,`tag`) VALUES ";
				$sql .= "($instrument_id, $language_id, '$tag')";
				$this->db->query($sql);
			} else {
				if ($exist_meta_title) {
					$sql  = "INSERT INTO `".DB_PREFIX."instrument_description` (`instrument_id`, `language_id`, `name`, `description`, `meta_title`, `meta_description`, `meta_keyword`, `tag`) VALUES ";
					$sql .= "( $instrument_id, $language_id, '$name', '$description', '$meta_title', '$meta_description', '$meta_keyword', '$tag' );";
				} else {
					$sql  = "INSERT INTO `".DB_PREFIX."instrument_description` (`instrument_id`, `language_id`, `name`, `description`, `meta_description`, `meta_keyword`, `tag`) VALUES ";
					$sql .= "( $instrument_id, $language_id, '$name', '$description', '$meta_description', '$meta_keyword', '$tag' );";
				}
				$this->db->query( $sql );
			}
		}
		if (count($categories) > 0) {
			$sql = "INSERT INTO `".DB_PREFIX."instrument_to_category` (`instrument_id`,`category_id`) VALUES ";
			$first = true;
			foreach ($categories as $category_id) {
				$sql .= ($first) ? "\n" : ",\n";
				$first = false;
				$sql .= "($instrument_id,$category_id)";
			}
			$sql .= ";";
			$this->db->query($sql);
		}
		if ($keyword) {
			if (isset($url_alias_ids[$instrument_id])) {
				$url_alias_id = $url_alias_ids[$instrument_id];
				$sql = "INSERT INTO `".DB_PREFIX."url_alias` (`url_alias_id`,`query`,`keyword`) VALUES ($url_alias_id,'instrument_id=$instrument_id','$keyword');";
				unset($url_alias_ids[$instrument_id]);
			} else {
				$sql = "INSERT INTO `".DB_PREFIX."url_alias` (`query`,`keyword`) VALUES ('instrument_id=$instrument_id','$keyword');";
			}
			$this->db->query($sql);
		}
		foreach ($store_ids as $store_id) {
			if (in_array((int)$store_id,$available_store_ids)) {
				$sql = "INSERT INTO `".DB_PREFIX."instrument_to_store` (`instrument_id`,`store_id`) VALUES ($instrument_id,$store_id);";
				$this->db->query($sql);
			}
		}
		$layouts = array();
		...
		foreach ($layouts as $store_id => $layout_id) {
			$sql = "INSERT INTO `".DB_PREFIX."instrument_to_layout` (`instrument_id`,`store_id`,`layout_id`) VALUES ($instrument_id,$store_id,$layout_id);";
			$this->db->query($sql);
		}
		if (count($related_ids) > 0) {
			$sql = "INSERT INTO `".DB_PREFIX."instrument_related` (`instrument_id`,`related_id`) VALUES ";
			$first = true;
			foreach ($related_ids as $related_id) {
				$sql .= ($first) ? "\n" : ",\n";
				$first = false;
				$sql .= "($instrument_id,$related_id)";
			}
			$sql .= ";";
			$this->db->query($sql);
		}
	...
	protected function deleteInstruments( $exist_table_instrument_tag, &$url_alias_ids ) {
		$sql  = "TRUNCATE TABLE `".DB_PREFIX."instrument`;\n";
		$sql .= "TRUNCATE TABLE `".DB_PREFIX."instrument_description`;\n";
		$sql .= "TRUNCATE TABLE `".DB_PREFIX."instrument_to_category`;\n";
		$sql .= "TRUNCATE TABLE `".DB_PREFIX."instrument_to_store`;\n";
		$sql .= "DELETE FROM `".DB_PREFIX."url_alias` WHERE `query` LIKE 'instrument_id=%';\n";
		$sql .= "TRUNCATE TABLE `".DB_PREFIX."instrument_related`;\n";
		$sql .= "TRUNCATE TABLE `".DB_PREFIX."instrument_to_layout`;\n";
		if ($exist_table_instrument_tag) {
			$sql .= "TRUNCATE TABLE `".DB_PREFIX."instrument_tag`;\n";
		}
		$this->multiquery( $sql );
		$sql = "SELECT (MAX(url_alias_id)+1) AS next_url_alias_id FROM `".DB_PREFIX."url_alias` LIMIT 1";
		$query = $this->db->query( $sql );
		...
		$this->db->query( $sql );
		$remove = array();
		foreach ($url_alias_ids as $instrument_id=>$url_alias_id) {
			if ($url_alias_id >= $next_url_alias_id) {
				$remove[$instrument_id] = $url_alias_id;
			}
		}
		foreach ($remove as $instrument_id=>$url_alias_id) {
			unset($url_alias_ids[$instrument_id]);
		}
	}


	protected function deleteInstrument( $instrument_id, $exist_table_instrument_tag ) {
		$sql  = "DELETE FROM `".DB_PREFIX."instrument` WHERE `instrument_id` = '$instrument_id';\n";
		$sql .= "DELETE FROM `".DB_PREFIX."instrument_description` WHERE `instrument_id` = '$instrument_id';\n";
		$sql .= "DELETE FROM `".DB_PREFIX."instrument_to_category` WHERE `instrument_id` = '$instrument_id';\n";
		$sql .= "DELETE FROM `".DB_PREFIX."instrument_to_store` WHERE `instrument_id` = '$instrument_id';\n";
		$sql .= "DELETE FROM `".DB_PREFIX."url_alias` WHERE `query` LIKE 'instrument_id=$instrument_id';\n";
		$sql .= "DELETE FROM `".DB_PREFIX."instrument_related` WHERE `instrument_id` = '$instrument_id';\n";
		$sql .= "DELETE FROM `".DB_PREFIX."instrument_to_layout` WHERE `instrument_id` = '$instrument_id';\n";
		if ($exist_table_instrument_tag) {
			$sql .= "DELETE FROM `".DB_PREFIX."instrument_tag` WHERE `instrument_id` = '$instrument_id';\n";
		}
		$this->multiquery( $sql );
	}
	...
	// function for reading additional cells in class extensions
	protected function moreInstrumentCells( $i, &$j, &$worksheet, &$instrument ) {
		return;
	}


	protected function uploadInstruments( &$reader, $incremental, &$available_instrument_ids=array() ) {
		// get worksheet, if not there return immediately
		$data = $reader->getSheetByName( 'Instruments' );
		if ($data==null) {
			return;
		}

		// save instrument view counts
		$view_counts = $this->getInstrumentViewCounts();

		// save old url_alias_ids
		$url_alias_ids = $this->getInstrumentUrlAliasIds();

		// some older versions of OpenCart use the 'instrument_tag' table
		$exist_table_instrument_tag = false;
		$query = $this->db->query( "SHOW TABLES LIKE '".DB_PREFIX."instrument_tag'" );
		$exist_table_instrument_tag = ($query->num_rows > 0);
		...
		// if incremental then find current instrument IDs else delete all old instruments
		$available_instrument_ids = array();
		if ($incremental) {
			$old_instrument_ids = $this->getAvailableInstrumentIds($data);
		} else {
			$this->deleteInstruments($exist_table_instrument_tag,$url_alias_ids);
		}
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$instrument['categories'] = ($categories=="") ? array() : explode( ",", $categories );
			if ($instrument['categories']===false) {
				$instrument['categories'] = array();
			}
			$instrument['quantity'] = $quantity;
			$instrument['model'] = $model;
			$instrument['manufacturer_name'] = $manufacturer_name;
			$instrument['image'] = $image_name;
			$instrument['shipping'] = $shipping;
			$instrument['price'] = $price;
			$instrument['points'] = $points;
			$instrument['date_added'] = $date_added;
			$instrument['date_modified'] = $date_modified;
			$instrument['date_available'] = $date_available;
			$instrument['weight'] = $weight;
			$instrument['weight_unit'] = $weight_unit;
			$instrument['status'] = $status;
			$instrument['tax_class_id'] = $tax_class_id;
			$instrument['viewed'] = isset($view_counts[$instrument_id]) ? $view_counts[$instrument_id] : 0;
			$instrument['descriptions'] = $descriptions;
			$instrument['stock_status_id'] = $stock_status_id;
			if ($exist_meta_title) {
				$instrument['meta_titles'] = $meta_titles;
			}
			$instrument['meta_descriptions'] = $meta_descriptions;
			$instrument['length'] = $length;
			$instrument['width'] = $width;
			$instrument['height'] = $height;
			$instrument['seo_keyword'] = $keyword;
			$instrument['measurement_unit'] = $measurement_unit;
			$instrument['sku'] = $sku;
			$instrument['upc'] = $upc;
			if (in_array('ean',$instrument_fields)) {
				$instrument['ean'] = $ean;
			...
			$store_ids = trim( $this->clean($store_ids, false) );
			$instrument['store_ids'] = ($store_ids=="") ? array() : explode( ",", $store_ids );
			if ($instrument['store_ids']===false) {
				$instrument['store_ids'] = array();
			}
			$instrument['related_ids'] = ($related=="") ? array() : explode( ",", $related );
			if ($instrument['related_ids']===false) {
				$instrument['related_ids'] = array();
			}
			$instrument['layout'] = ($layout=="") ? array() : explode( ",", $layout );
			if ($instrument['layout']===false) {
				$instrument['layout'] = array();
			}
			$instrument['subtract'] = $subtract;
			$instrument['minimum'] = $minimum;
			$instrument['meta_keywords'] = $meta_keywords;
			$instrument['tags'] = $tags;
			$instrument['sort_order'] = $sort_order;
			if ($incremental) {
				$this->deleteInstrument( $instrument_id, $exist_table_instrument_tag );
			}
			$available_instrument_ids[$instrument_id] = $instrument_id;
			$this->moreInstrumentCells( $i, $j, $data, $instrument );
			$this->storeInstrumentIntoDatabase( $instrument, $languages, $instrument_fields, $exist_table_instrument_tag, $exist_meta_title, $layout_ids, $available_store_ids, $manufacturers, $weight_class_ids, $length_class_ids, $url_alias_ids );
		}
	}


	protected function storeAdditionalImageIntoDatabase( &$image, &$old_instrument_image_ids, $exist_sort_order=true ) {
		$instrument_id = $image['instrument_id'];
		$image_name = $image['image_name'];
		if ($exist_sort_order) {
			$sort_order = $image['sort_order'];
		}
		if (isset($old_instrument_image_ids[$instrument_id][$image_name])) {
			$instrument_image_id = $old_instrument_image_ids[$instrument_id][$image_name];
			if ($exist_sort_order) {
				$sql  = "INSERT INTO `".DB_PREFIX."instrument_image` (`instrument_image_id`,`instrument_id`,`image`,`sort_order` ) VALUES "; 
				$sql .= "($instrument_image_id,$instrument_id,'".$this->db->escape($image_name)."',$sort_order)";
			} else {
				$sql  = "INSERT INTO `".DB_PREFIX."instrument_image` (`instrument_image_id`,`instrument_id`,`image` ) VALUES "; 
				$sql .= "($instrument_image_id,$instrument_id,'".$this->db->escape($image_name)."')";
			}
			$this->db->query($sql);
			unset($old_instrument_image_ids[$instrument_id][$image_name]);
		} else {
			if ($exist_sort_order) {
				$sql  = "INSERT INTO `".DB_PREFIX."instrument_image` (`instrument_id`,`image`,`sort_order` ) VALUES "; 
				$sql .= "($instrument_id,'".$this->db->escape($image_name)."',$sort_order)";
			} else {
				$sql  = "INSERT INTO `".DB_PREFIX."instrument_image` (`instrument_id`,`image` ) VALUES "; 
				$sql .= "($instrument_id,'".$this->db->escape($image_name)."')";
			}
			$this->db->query($sql);
		}
	}
	...
	protected function deleteAdditionalImages() {
		$sql = "TRUNCATE TABLE `".DB_PREFIX."instrument_image`";
		$this->db->query( $sql );
	}


	protected function deleteAdditionalImage( $instrument_id ) {
		$sql = "SELECT instrument_image_id, instrument_id, image FROM `".DB_PREFIX."instrument_image` WHERE instrument_id='".(int)$instrument_id."'";
		$query = $this->db->query( $sql );
		$old_instrument_image_ids = array();
		foreach ($query->rows as $row) {
			$instrument_image_id = $row['instrument_image_id'];
			$instrument_id = $row['instrument_id'];
			$image_name = $row['image'];
			$old_instrument_image_ids[$instrument_id][$image_name] = $instrument_image_id;
		}
		if ($old_instrument_image_ids) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_image` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
		return $old_instrument_image_ids;
	}


	protected function deleteUnlistedAdditionalImages( &$unlisted_instrument_ids ) {
		foreach ($unlisted_instrument_ids as $instrument_id) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_image` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
	}
	...
	protected function uploadAdditionalImages( &$reader, $incremental, &$available_instrument_ids ) {
		// get worksheet, if not there return immediately
		...
		// if incremental then find current instrument IDs else delete all old additional images
		if ($incremental) {
		...
		// check for the existence of instrument_image.sort_order field
		$sql = "SHOW COLUMNS FROM `".DB_PREFIX."instrument_image` LIKE 'sort_order'";
		$query = $this->db->query( $sql );
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			if (($incremental) && ($instrument_id != $previous_instrument_id)) {
				$old_instrument_image_ids = $this->deleteAdditionalImage( $instrument_id );
			...
			$this->storeAdditionalImageIntoDatabase( $image, $old_instrument_image_ids, $exist_sort_order );
			$previous_instrument_id = $instrument_id;
		}
		if ($incremental) {
			$this->deleteUnlistedAdditionalImages( $unlisted_instrument_ids );
		}
	}


	protected function storeSpecialIntoDatabase( &$special, &$old_instrument_special_ids, &$user_group_ids ) {
		$instrument_id = $special['instrument_id'];
		$name = $special['user_group'];
		$user_group_id = isset($user_group_ids[$name]) ? $user_group_ids[$name] : $this->config->get('config_user_group_id');
		$priority = $special['priority'];
		$price = $special['price'];
		$date_start = $special['date_start'];
		$date_end = $special['date_end'];
		if (isset($old_instrument_special_ids[$instrument_id][$user_group_id])) {
			$instrument_special_id = $old_instrument_special_ids[$instrument_id][$user_group_id];
			$sql  = "INSERT INTO `".DB_PREFIX."instrument_special` (`instrument_special_id`,`instrument_id`,`user_group_id`,`priority`,`price`,`date_start`,`date_end` ) VALUES "; 
			$sql .= "($instrument_special_id,$instrument_id,$user_group_id,$priority,$price,'$date_start','$date_end')";
			$this->db->query($sql);
			unset($old_instrument_special_ids[$instrument_id][$user_group_id]);
		} else {
			$sql  = "INSERT INTO `".DB_PREFIX."instrument_special` (`instrument_id`,`user_group_id`,`priority`,`price`,`date_start`,`date_end` ) VALUES "; 
			$sql .= "($instrument_id,$user_group_id,$priority,$price,'$date_start','$date_end')";
			$this->db->query($sql);
		}
	}
	...
	protected function deleteSpecials() {
		$sql = "TRUNCATE TABLE `".DB_PREFIX."instrument_special`";
		$this->db->query( $sql );
	}


	protected function deleteSpecial( $instrument_id ) {
		$sql = "SELECT instrument_special_id, instrument_id, user_group_id FROM `".DB_PREFIX."instrument_special` WHERE instrument_id='".(int)$instrument_id."'";
		$query = $this->db->query( $sql );
		$old_instrument_special_ids = array();
		foreach ($query->rows as $row) {
			$instrument_special_id = $row['instrument_special_id'];
			$instrument_id = $row['instrument_id'];
			$user_group_id = $row['user_group_id'];
			$old_instrument_special_ids[$instrument_id][$user_group_id] = $instrument_special_id;
		}
		if ($old_instrument_special_ids) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_special` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
		return $old_instrument_special_ids;
	}


	protected function deleteUnlistedSpecials( &$unlisted_instrument_ids ) {
		foreach ($unlisted_instrument_ids as $instrument_id) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_special` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
	...
	protected function uploadSpecials( &$reader, $incremental, &$available_instrument_ids ) {
		// get worksheet, if not there return immediately
		...
		// if incremental then find current instrument IDs else delete all old specials
		if ($incremental) {
			$unlisted_instrument_ids = $available_instrument_ids;
		} else {
			$this->deleteSpecials();
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$special = array();
			$special['instrument_id'] = $instrument_id;
			$special['user_group'] = $user_group;
			$special['priority'] = $priority;
			$special['price'] = $price;
			$special['date_start'] = $date_start;
			$special['date_end'] = $date_end;
			if (($incremental) && ($instrument_id != $previous_instrument_id)) {
				$old_instrument_special_ids = $this->deleteSpecial( $instrument_id );
				if (isset($unlisted_instrument_ids[$instrument_id])) {
					unset($unlisted_instrument_ids[$instrument_id]);
				}
			}
			$this->moreSpecialCells( $i, $j, $data, $special );
			$this->storeSpecialIntoDatabase( $special, $old_instrument_special_ids, $user_group_ids );
			$previous_instrument_id = $instrument_id;
		}
		if ($incremental) {
			$this->deleteUnlistedSpecials( $unlisted_instrument_ids );
		}
	}


	protected function storeDiscountIntoDatabase( &$discount, &$old_instrument_discount_ids, &$user_group_ids ) {
		$instrument_id = $discount['instrument_id'];
		$name = $discount['user_group'];
		$user_group_id = isset($user_group_ids[$name]) ? $user_group_ids[$name] : $this->config->get('config_user_group_id');
		...
		if (isset($old_instrument_discount_ids[$instrument_id][$user_group_id][$quantity])) {
			$instrument_discount_id = $old_instrument_discount_ids[$instrument_id][$user_group_id][$quantity];
			$sql  = "INSERT INTO `".DB_PREFIX."instrument_discount` (`instrument_discount_id`,`instrument_id`,`user_group_id`,`quantity`,`priority`,`price`,`date_start`,`date_end` ) VALUES "; 
			$sql .= "($instrument_discount_id,$instrument_id,$user_group_id,$quantity,$priority,$price,'$date_start','$date_end')";
			$this->db->query($sql);
			unset($old_instrument_discount_ids[$instrument_id][$user_group_id][$quantity]);
		} else {
			$sql  = "INSERT INTO `".DB_PREFIX."instrument_discount` (`instrument_id`,`user_group_id`,`quantity`,`priority`,`price`,`date_start`,`date_end` ) VALUES "; 
			$sql .= "($instrument_id,$user_group_id,$quantity,$priority,$price,'$date_start','$date_end')";
			$this->db->query($sql);
		}
	}
	...
	protected function deleteDiscounts() {
		$sql = "TRUNCATE TABLE `".DB_PREFIX."instrument_discount`";
		$this->db->query( $sql );
	}


	protected function deleteDiscount( $instrument_id ) {
		$sql = "SELECT instrument_discount_id, instrument_id, user_group_id, quantity FROM `".DB_PREFIX."instrument_discount` WHERE instrument_id='".(int)$instrument_id."' ORDER BY instrument_id ASC, user_group_id ASC, quantity ASC;";
		$query = $this->db->query( $sql );
		$old_instrument_discount_ids = array();
		foreach ($query->rows as $row) {
			$instrument_discount_id = $row['instrument_discount_id'];
			$instrument_id = $row['instrument_id'];
			$user_group_id = $row['user_group_id'];
			$quantity = $row['quantity'];
			$old_instrument_discount_ids[$instrument_id][$user_group_id][$quantity] = $instrument_discount_id;
		}
		if ($old_instrument_discount_ids) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_discount` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
		return $old_instrument_discount_ids;
	}


	protected function deleteUnlistedDiscounts( &$unlisted_instrument_ids ) {
		foreach ($unlisted_instrument_ids as $instrument_id) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_discount` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
	...
	protected function uploadDiscounts( &$reader, $incremental, &$available_instrument_ids ) {
		// get worksheet, if not there return immediately
		...
		// if incremental then find current instrument IDs else delete all old discounts
		if ($incremental) {
			$unlisted_instrument_ids = $available_instrument_ids;
		} else {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$date_end = $this->getCell($data,$i,$j++,'0000-00-00');
			$discount = array();
			$discount['instrument_id'] = $instrument_id;
			$discount['user_group'] = $user_group;
			...
			if (($incremental) && ($instrument_id != $previous_instrument_id)) {
				$old_instrument_discount_ids = $this->deleteDiscount( $instrument_id );
				if (isset($unlisted_instrument_ids[$instrument_id])) {
					unset($unlisted_instrument_ids[$instrument_id]);
				}
			}
			$this->moreDiscountCells( $i, $j, $data, $discount );
			$this->storeDiscountIntoDatabase( $discount, $old_instrument_discount_ids, $user_group_ids );
			$previous_instrument_id = $instrument_id;
		}
		if ($incremental) {
			$this->deleteUnlistedDiscounts( $unlisted_instrument_ids );
		}
	}


	protected function storeRewardIntoDatabase( &$reward, &$old_instrument_reward_ids, &$user_group_ids ) {
		$instrument_id = $reward['instrument_id'];
		$name = $reward['user_group'];
		$user_group_id = isset($user_group_ids[$name]) ? $user_group_ids[$name] : $this->config->get('config_user_group_id');
		$points = $reward['points'];
		if (isset($old_instrument_reward_ids[$instrument_id][$user_group_id])) {
			$instrument_reward_id = $old_instrument_reward_ids[$instrument_id][$user_group_id];
			$sql  = "INSERT INTO `".DB_PREFIX."instrument_reward` (`instrument_reward_id`,`instrument_id`,`user_group_id`,`points` ) VALUES "; 
			$sql .= "($instrument_reward_id,$instrument_id,$user_group_id,$points)";
			$this->db->query($sql);
			unset($old_instrument_reward_ids[$instrument_id][$user_group_id]);
		} else {
			$sql  = "INSERT INTO `".DB_PREFIX."instrument_reward` (`instrument_id`,`user_group_id`,`points` ) VALUES "; 
			$sql .= "($instrument_id,$user_group_id,$points)";
			$this->db->query($sql);
		}
	}
	...
	protected function deleteRewards() {
		$sql = "TRUNCATE TABLE `".DB_PREFIX."instrument_reward`";
		$this->db->query( $sql );
	}


	protected function deleteReward( $instrument_id ) {
		$sql = "SELECT instrument_reward_id, instrument_id, user_group_id FROM `".DB_PREFIX."instrument_reward` WHERE instrument_id='".(int)$instrument_id."'";
		$query = $this->db->query( $sql );
		$old_instrument_reward_ids = array();
		foreach ($query->rows as $row) {
			$instrument_reward_id = $row['instrument_reward_id'];
			$instrument_id = $row['instrument_id'];
			$user_group_id = $row['user_group_id'];
			$old_instrument_reward_ids[$instrument_id][$user_group_id] = $instrument_reward_id;
		}
		if ($old_instrument_reward_ids) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_reward` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
		return $old_instrument_reward_ids;
	}


	protected function deleteUnlistedRewards( &$unlisted_instrument_ids ) {
		foreach ($unlisted_instrument_ids as $instrument_id) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_reward` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
	...
	protected function storeInstrumentOptionIntoDatabase( &$instrument_option, &$old_instrument_option_ids ) {
		// Opencart versions from 2.0 onwards use instrument_option.value instead of the older instrument_option.option_value
		$sql = "SHOW COLUMNS FROM `".DB_PREFIX."instrument_option` LIKE 'value'";
		$query = $this->db->query( $sql );
		$exist_po_value = ($query->num_rows > 0) ? true : false;

		// DB query for storing the instrument option
		$instrument_id = $instrument_option['instrument_id'];
		$option_id = $instrument_option['option_id'];
		$option_value = $instrument_option['option_value'];
		...
		if (isset($old_instrument_option_ids[$instrument_id][$option_id])) {
			$instrument_option_id = $old_instrument_option_ids[$instrument_id][$option_id];
			if ($exist_po_value) {
				$sql  = "INSERT INTO `".DB_PREFIX."instrument_option` (`instrument_option_id`,`instrument_id`,`option_id`,`value`,`required` ) VALUES "; 
			} else {
				$sql  = "INSERT INTO `".DB_PREFIX."instrument_option` (`instrument_option_id`,`instrument_id`,`option_id`,`option_value`,`required` ) VALUES "; 
			}
			$sql .= "($instrument_option_id,$instrument_id,$option_id,'".$this->db->escape($option_value)."',$required)";
			$this->db->query($sql);
			unset($old_instrument_option_ids[$instrument_id][$option_id]);
		} else {
			if ($exist_po_value) {
				$sql  = "INSERT INTO `".DB_PREFIX."instrument_option` (`instrument_id`,`option_id`,`value`,`required` ) VALUES "; 
			} else {
				$sql  = "INSERT INTO `".DB_PREFIX."instrument_option` (`instrument_id`,`option_id`,`option_value`,`required` ) VALUES "; 
			}
			$sql .= "($instrument_id,$option_id,'".$this->db->escape($option_value)."',$required)";
			$this->db->query($sql);
		}
	}


	protected function deleteInstrumentOptions() {
		$sql = "TRUNCATE TABLE `".DB_PREFIX."instrument_option`";
		$this->db->query( $sql );
	}


	protected function deleteInstrumentOption( $instrument_id ) {
		$sql = "SELECT instrument_option_id, instrument_id, option_id FROM `".DB_PREFIX."instrument_option` WHERE instrument_id='".(int)$instrument_id."'";
		$query = $this->db->query( $sql );
		$old_instrument_option_ids = array();
		foreach ($query->rows as $row) {
			$instrument_option_id = $row['instrument_option_id'];
			$instrument_id = $row['instrument_id'];
			$option_id = $row['option_id'];
			$old_instrument_option_ids[$instrument_id][$option_id] = $instrument_option_id;
		}
		if ($old_instrument_option_ids) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_option` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
		return $old_instrument_option_ids;
	}


	protected function deleteUnlistedInstrumentOptions( &$unlisted_instrument_ids ) {
		foreach ($unlisted_instrument_ids as $instrument_id) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_option` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
	...
	// function for reading additional cells in class extensions
	protected function moreInstrumentOptionCells( $i, &$j, &$worksheet, &$instrument_option ) {
		return;
	}


	protected function uploadInstrumentOptions( &$reader, $incremental, &$available_instrument_ids ) {
		// get worksheet, if not there return immediately
		$data = $reader->getSheetByName( 'InstrumentOptions' );
		if ($data==null) {
			return;
		}

		// if incremental then find current instrument IDs else delete all old instrument options
		if ($incremental) {
			$unlisted_instrument_ids = $available_instrument_ids;
		} else {
			$this->deleteInstrumentOptions();
		}
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$instrument_option['instrument_id'] = $instrument_id;
			$instrument_option['option_id'] = $option_id;
			$instrument_option['option_value'] = $option_value;
			$instrument_option['required'] = $required;
			if (($incremental) && ($instrument_id != $previous_instrument_id)) {
				$old_instrument_option_ids = $this->deleteInstrumentOption( $instrument_id );
				if (isset($unlisted_instrument_ids[$instrument_id])) {
					unset($unlisted_instrument_ids[$instrument_id]);
				}
			}
			$this->moreInstrumentOptionCells( $i, $j, $data, $instrument_option );
			$this->storeInstrumentOptionIntoDatabase( $instrument_option, $old_instrument_option_ids );
			$previous_instrument_id = $instrument_id;
		}
		if ($incremental) {
			$this->deleteUnlistedInstrumentOptions( $unlisted_instrument_ids );
		}
	}
	...
	protected function getInstrumentOptionIds( $instrument_id ) {
		$sql  = "SELECT instrument_option_id, option_id FROM `".DB_PREFIX."instrument_option` ";
		$sql .= "WHERE instrument_id='".(int)$instrument_id."'";
		$query = $this->db->query( $sql );
		$instrument_option_ids = array();
		foreach ($query->rows as $row) {
			$instrument_option_id = $row['instrument_option_id'];
			$option_id = $row['option_id'];
			$instrument_option_ids[$option_id] = $instrument_option_id;
		}
		return $instrument_option_ids;
	}


	protected function storeInstrumentOptionValueIntoDatabase( &$instrument_option_value, &$old_instrument_option_value_ids ) {
		$instrument_id = $instrument_option_value['instrument_id'];
		$option_id = $instrument_option_value['option_id'];
		...
		$instrument_option_id = $instrument_option_value['instrument_option_id'];
		if (isset($old_instrument_option_value_ids[$instrument_id][$option_id][$option_value_id])) {
			$instrument_option_value_id = $old_instrument_option_value_ids[$instrument_id][$option_id][$option_value_id];
			$sql  = "INSERT INTO `".DB_PREFIX."instrument_option_value` ";
			$sql .= "(`instrument_option_value_id`,`instrument_option_id`,`instrument_id`,`option_id`,`option_value_id`,`quantity`,`subtract`,`price`,`price_prefix`,`points`,`points_prefix`,`weight`,`weight_prefix` ) VALUES "; 
			$sql .= "($instrument_option_value_id,$instrument_option_id,$instrument_id,$option_id,$option_value_id,$quantity,$subtract,$price,'$price_prefix',$points,'$points_prefix',$weight,'$weight_prefix')";
			$this->db->query($sql);
			unset($old_instrument_option_value_ids[$instrument_id][$option_id][$option_value_id]);
		} else {
			$sql  = "INSERT INTO `".DB_PREFIX."instrument_option_value` ";
			$sql .= "(`instrument_option_id`,`instrument_id`,`option_id`,`option_value_id`,`quantity`,`subtract`,`price`,`price_prefix`,`points`,`points_prefix`,`weight`,`weight_prefix` ) VALUES "; 
			$sql .= "($instrument_option_id,$instrument_id,$option_id,$option_value_id,$quantity,$subtract,$price,'$price_prefix',$points,'$points_prefix',$weight,'$weight_prefix')";
			$this->db->query($sql);
		}
	}


	protected function deleteInstrumentOptionValues() {
		$sql = "TRUNCATE TABLE `".DB_PREFIX."instrument_option_value`";
		$this->db->query( $sql );
	}


	protected function deleteInstrumentOptionValue( $instrument_id ) {
		$sql = "SELECT instrument_option_value_id, instrument_id, option_id, option_value_id FROM `".DB_PREFIX."instrument_option_value` WHERE instrument_id='".(int)$instrument_id."'";
		$query = $this->db->query( $sql );
		$old_instrument_option_value_ids = array();
		foreach ($query->rows as $row) {
			$instrument_option_value_id = $row['instrument_option_value_id'];
			$instrument_id = $row['instrument_id'];
			$option_id = $row['option_id'];
			$option_value_id = $row['option_value_id'];
			$old_instrument_option_value_ids[$instrument_id][$option_id][$option_value_id] = $instrument_option_value_id;
		}
		if ($old_instrument_option_value_ids) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_option_value` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
		return $old_instrument_option_value_ids;
	}


	protected function deleteUnlistedInstrumentOptionValues( &$unlisted_instrument_ids ) {
		foreach ($unlisted_instrument_ids as $instrument_id) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_option_value` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
	...
	// function for reading additional cells in class extensions
	protected function moreInstrumentOptionValueCells( $i, &$j, &$worksheet, &$instrument_option_value ) {
		return;
	}


	protected function uploadInstrumentOptionValues( &$reader, $incremental, &$available_instrument_ids ) {
		// get worksheet, if not there return immediately
		$data = $reader->getSheetByName( 'InstrumentOptionValues' );
		if ($data==null) {
			return;
		}

		// if incremental then find current instrument IDs else delete all old instrument option values
		if ($incremental) {
			$unlisted_instrument_ids = $available_instrument_ids;
		} else {
			$this->deleteInstrumentOptionValues();
		}
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			if ($instrument_id != $previous_instrument_id) {
				$instrument_option_ids = $this->getInstrumentOptionIds( $instrument_id );
			}
			$instrument_option_value = array();
			$instrument_option_value['instrument_id'] = $instrument_id;
			$instrument_option_value['option_id'] = $option_id;
			$instrument_option_value['option_value_id'] = $option_value_id;
			$instrument_option_value['quantity'] = $quantity;
			$instrument_option_value['subtract'] = $subtract;
			$instrument_option_value['price'] = $price;
			$instrument_option_value['price_prefix'] = $price_prefix;
			$instrument_option_value['points'] = $points;
			$instrument_option_value['points_prefix'] = $points_prefix;
			$instrument_option_value['weight'] = $weight;
			$instrument_option_value['weight_prefix'] = $weight_prefix;
			$instrument_option_value['instrument_option_id'] = isset($instrument_option_ids[$option_id]) ? $instrument_option_ids[$option_id] : 0;
			if (($incremental) && ($instrument_id != $previous_instrument_id)) {
				$old_instrument_option_value_ids = $this->deleteInstrumentOptionValue( $instrument_id );
				if (isset($unlisted_instrument_ids[$instrument_id])) {
					unset($unlisted_instrument_ids[$instrument_id]);
				}
			}
			$this->moreInstrumentOptionValueCells( $i, $j, $data, $instrument_option_value );
			$this->storeInstrumentOptionValueIntoDatabase( $instrument_option_value, $old_instrument_option_value_ids );
			$previous_instrument_id = $instrument_id;
		}
		if ($incremental) {
			$this->deleteUnlistedInstrumentOptionValues( $unlisted_instrument_ids );
		}
	}
	...
	protected function storeInstrumentAttributeIntoDatabase( &$instrument_attribute, &$languages ) {
		$instrument_id = $instrument_attribute['instrument_id'];
		$attribute_id = $instrument_attribute['attribute_id'];
		$texts = $instrument_attribute['texts'];
		foreach ($languages as $language) {
			$language_code = $language['code'];
			$language_id = $language['language_id'];
			$text = isset($texts[$language_code]) ? $this->db->escape($texts[$language_code]) : '';
			$sql  = "INSERT INTO `".DB_PREFIX."instrument_attribute` (`instrument_id`, `attribute_id`, `language_id`, `text`) VALUES ";
			$sql .= "( $instrument_id, $attribute_id, $language_id, '$text' );";
			$this->db->query( $sql );
		}
	}


	protected function deleteInstrumentAttributes() {
		$sql = "TRUNCATE TABLE `".DB_PREFIX."instrument_attribute`";
		$this->db->query( $sql );
	}


	protected function deleteInstrumentAttribute( $instrument_id ) {
		$sql = "DELETE FROM `".DB_PREFIX."instrument_attribute` WHERE instrument_id='".(int)$instrument_id."'";
		$this->db->query( $sql );
	}


	protected function deleteUnlistedInstrumentAttributes( &$unlisted_instrument_ids ) {
		foreach ($unlisted_instrument_ids as $instrument_id) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_attribute` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
	...
	// function for reading additional cells in class extensions
	protected function moreInstrumentAttributeCells( $i, &$j, &$worksheet, &$instrument_attribute ) {
		return;
	}


	protected function uploadInstrumentAttributes( &$reader, $incremental, &$available_instrument_ids ) {
		// get worksheet, if not there return immediately
		$data = $reader->getSheetByName( 'InstrumentAttributes' );
		if ($data==null) {
			return;
		}

		// if incremental then find current instrument IDs else delete all old instrument attributes
		if ($incremental) {
			$unlisted_instrument_ids = $available_instrument_ids;
		} else {
			$this->deleteInstrumentAttributes();
		}
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$instrument_attribute = array();
			$instrument_attribute['instrument_id'] = $instrument_id;
			$instrument_attribute['attribute_group_id'] = $attribute_group_id;
			$instrument_attribute['attribute_id'] = $attribute_id;
			$instrument_attribute['texts'] = $texts;
			if (($incremental) && ($instrument_id != $previous_instrument_id)) {
				$this->deleteInstrumentAttribute( $instrument_id );
				if (isset($unlisted_instrument_ids[$instrument_id])) {
					unset($unlisted_instrument_ids[$instrument_id]);
				}
			}
			$this->moreInstrumentAttributeCells( $i, $j, $data, $instrument_attribute );
			$this->storeInstrumentAttributeIntoDatabase( $instrument_attribute, $languages );
			$previous_instrument_id = $instrument_id;
		}
		if ($incremental) {
			$this->deleteUnlistedInstrumentAttributes( $unlisted_instrument_ids );
		}
	}
	...
	protected function storeInstrumentFilterIntoDatabase( &$instrument_filter, &$languages ) {
		$instrument_id = $instrument_filter['instrument_id'];
		$filter_id = $instrument_filter['filter_id'];
		$sql  = "INSERT INTO `".DB_PREFIX."instrument_filter` (`instrument_id`, `filter_id`) VALUES ";
		$sql .= "( $instrument_id, $filter_id );";
		$this->db->query( $sql );
	}


	protected function deleteInstrumentFilters() {
		$sql = "TRUNCATE TABLE `".DB_PREFIX."instrument_filter`";
		$this->db->query( $sql );
	}


	protected function deleteInstrumentFilter( $instrument_id ) {
		$sql = "DELETE FROM `".DB_PREFIX."instrument_filter` WHERE instrument_id='".(int)$instrument_id."'";
		$this->db->query( $sql );
	}


	protected function deleteUnlistedInstrumentFilters( &$unlisted_instrument_ids ) {
		foreach ($unlisted_instrument_ids as $instrument_id) {
			$sql = "DELETE FROM `".DB_PREFIX."instrument_filter` WHERE instrument_id='".(int)$instrument_id."'";
			$this->db->query( $sql );
		}
	...
	// function for reading additional cells in class extensions
	protected function moreInstrumentFilterCells( $i, &$j, &$worksheet, &$instrument_filter ) {
		return;
	}


	protected function uploadInstrumentFilters( &$reader, $incremental, &$available_instrument_ids ) {
		// get worksheet, if not there return immediately
		...
		// if incremental then find current instrument IDs else delete all old instrument filters
		if ($incremental) {
			$unlisted_instrument_ids = $available_instrument_ids;
		} else {
			$this->deleteInstrumentFilters();
		}
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$instrument_filter = array();
			$instrument_filter['instrument_id'] = $instrument_id;
			$instrument_filter['filter_group_id'] = $filter_group_id;
			$instrument_filter['filter_id'] = $filter_id;
			if (($incremental) && ($instrument_id != $previous_instrument_id)) {
				$this->deleteInstrumentFilter( $instrument_id );
				if (isset($unlisted_instrument_ids[$instrument_id])) {
					unset($unlisted_instrument_ids[$instrument_id]);
				}
			}
			$this->moreInstrumentFilterCells( $i, $j, $data, $instrument_filter );
			$this->storeInstrumentFilterIntoDatabase( $instrument_filter, $languages );
			$previous_instrument_id = $instrument_id;
		}
		if ($incremental) {
			$this->deleteUnlistedInstrumentFilters( $unlisted_instrument_ids );
		}
	}
	...
	protected function getAvailableUserIds() {
	...
	protected function storeUserIntoDatabase( &$user, $exist_custom_field, $exist_salt, $exist_safe, $exist_token, $exist_code ) {
		...
		$sql .= "(`user_id`,`user_group_id`,`store_id`,`firstname`,`lastname`,";
		...
		$sql .= "( $user_id, $user_group_id, $store_id, ";
	...
	protected function deleteUsers() {
		$sql = "TRUNCATE TABLE `".DB_PREFIX."user`";
		$this->db->query( $sql );
	}


	protected function deleteUser( $user_id ) {
		$sql = "DELETE FROM `".DB_PREFIX."user` WHERE user_id='".(int)$user_id."'";
		$this->db->query( $sql );
	}


	// function for reading additional cells in class extensions
	protected function moreUserCells( $i, &$j, &$worksheet, &$user ) {
		return;
	}


	protected function uploadUsers( &$reader, $incremental, &$available_user_ids=array() ) {
		// get worksheet, if not there return immediately
		$data = $reader->getSheetByName( 'Users' );
		if ($data==null) {
			return;
		}

		// Some fields are only available in newer Opencart versions
		$sql = "SHOW COLUMNS FROM `".DB_PREFIX."user` LIKE 'custom_field'";
		$query = $this->db->query( $sql );
		$exist_custom_field = ($query->num_rows > 0) ? true : false;
		$sql = "SHOW COLUMNS FROM `".DB_PREFIX."user` LIKE 'salt'";
		$query = $this->db->query( $sql );
		$exist_salt = ($query->num_rows > 0) ? true : false;
		$sql = "SHOW COLUMNS FROM `".DB_PREFIX."user` LIKE 'safe'";
		$query = $this->db->query( $sql );
		$exist_safe = ($query->num_rows > 0) ? true : false;
		$sql = "SHOW COLUMNS FROM `".DB_PREFIX."user` LIKE 'token'";
		$query = $this->db->query( $sql );
		$exist_token = ($query->num_rows > 0) ? true : false;
		$sql = "SHOW COLUMNS FROM `".DB_PREFIX."user` LIKE 'code'";
		$query = $this->db->query( $sql );
		$exist_code = ($query->num_rows > 0) ? true : false;

		// get user_group ids indexed by user group names
		$user_group_ids = $this->getUserGroupIds();

		// if incremental then find current user IDs else delete all old users
		$available_user_ids = array();
		if ($incremental) {
			$old_user_ids = $this->getAvailableUserIds();
		} else {
			$this->deleteUsers();
		}
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$j = 1;
			$user_id = trim($this->getCell($data,$i,$j++));
			if ($user_id=="") {
				continue;
			}
			$user_group = trim($this->getCell($data,$i,$j++));
			$user_group_id = isset($user_group_ids[$user_group]) ? $user_group_ids[$user_group] : '0';
			...
			if ($incremental) {
				if ($old_user_ids) {
					if (in_array((int)$user_id,$old_user_ids)) {
						$this->deleteUser( $user_id );
					}
				}
			}
			$available_user_ids[$user_id] = $user_id;
			$this->moreUserCells( $i, $j, $data, $user );
			$this->storeUserIntoDatabase( $user, $exist_custom_field, $exist_salt, $exist_safe, $exist_token, $exist_code );
		}
	}
	...
	protected function storeAddressIntoDatabase( &$address, $exist_company_id, $exist_tax_id, $exist_custom_field ) {
		...
		$sql .= "(`user_id`,`firstname`,`lastname`,`company`,";
		if ($exist_company_id) {
			$sql .= "`company_id`,";
		}
		if ($exist_tax_id) {
		...
		if ($default) {
			...
			$sql = "UPDATE `".DB_PREFIX."user` SET address_id='".(int)$address_id."' WHERE user_id='".(int)$user_id."'";
			$this->db->query($sql);
		}
	}
	...
	protected function deleteAddress( $user_id ) {
		$sql = "DELETE FROM `".DB_PREFIX."address` WHERE user_id='".(int)$user_id."'";
		$this->db->query( $sql );
	}


	protected function deleteUnlistedAddresses( &$unlisted_user_ids ) {
		foreach ($unlisted_user_ids as $user_id) {
			$sql = "DELETE FROM `".DB_PREFIX."address` WHERE user_id='".(int)$user_id."'";
			$this->db->query( $sql );
		}
	}
	...
	protected function uploadAddresses( &$reader, $incremental, &$available_user_ids ) {
		// get worksheet, if not there return immediately
		...
		// if incremental then find current user IDs else delete all old addresses
		if ($incremental) {
			$unlisted_user_ids = $available_user_ids;
		} else {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			if (($incremental) && ($user_id != $previous_user_id)) {
				$this->deleteAddress( $user_id );
				if (isset($unlisted_user_ids[$user_id])) {
					unset($unlisted_user_ids[$user_id]);
				}
			}
		...
		if ($incremental) {
			$this->deleteUnlistedAddresses( $unlisted_user_ids );
		}
	}
	...
	protected function validateInstruments( &$reader ) {
		$data = $reader->getSheetByName( 'Instruments' );
		if ($data==null) {
			return true;
		}
		...
		$expected_heading = array
		( "instrument_id", "name", "categories", "sku", "upc" );
		if (in_array("ean",$instrument_fields)) {
	...
	protected function validateAdditionalImages( &$reader ) {
		...
		$exist_sort_order = ($query->num_rows > 0) ? true : false;
		if ($exist_sort_order) {
			$expected_heading = array( "instrument_id", "image", "sort_order" );
		} else {
			$expected_heading = array( "instrument_id", "image" );
		}
		$expected_multilingual = array();
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	}
	...
	protected function validateSpecials( &$reader ) {
		...
		$expected_heading = array( "instrument_id", "user_group", "priority", "price", "date_start", "date_end" );
		$expected_multilingual = array();
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	}
	...
	protected function validateDiscounts( &$reader ) {
		...
		$expected_heading = array( "instrument_id", "user_group", "quantity", "priority", "price", "date_start", "date_end" );
		$expected_multilingual = array();
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	}
	...
	protected function validateRewards( &$reader ) {
		...
		$expected_heading = array( "instrument_id", "user_group", "points" );
		$expected_multilingual = array();
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	}


	protected function validateInstrumentOptions( &$reader ) {
		$data = $reader->getSheetByName( 'InstrumentOptions' );
		if ($data==null) {
			return true;
		}
		if ($this->config->get('export_import_settings_use_option_id')) {
			$expected_heading = array( "instrument_id", "option_id", "default_option_value", "required" );
		} else {
			$expected_heading = array( "instrument_id", "option", "default_option_value", "required" );
		}
		$expected_multilingual = array();
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	}
	...
	protected function validateInstrumentOptionValues( &$reader ) {
		$data = $reader->getSheetByName( 'InstrumentOptionValues' );
		if ($data==null) {
			return true;
		}
		if ($this->config->get('export_import_settings_use_option_id')) {
			if ($this->config->get('export_import_settings_use_option_value_id')) {
				$expected_heading = array( "instrument_id", "option_id", "option_value_id", "quantity", "subtract", "price", "price_prefix", "points", "points_prefix", "weight", "weight_prefix" );
			} else {
				$expected_heading = array( "instrument_id", "option_id", "option_value", "quantity", "subtract", "price", "price_prefix", "points", "points_prefix", "weight", "weight_prefix" );
			}
		} else {
			if ($this->config->get('export_import_settings_use_option_value_id')) {
				$expected_heading = array( "instrument_id", "option", "option_value_id", "quantity", "subtract", "price", "price_prefix", "points", "points_prefix", "weight", "weight_prefix" );
			} else {
				$expected_heading = array( "instrument_id", "option", "option_value", "quantity", "subtract", "price", "price_prefix", "points", "points_prefix", "weight", "weight_prefix" );
			}
		}
		$expected_multilingual = array();
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	...
	protected function validateInstrumentAttributes( &$reader ) {
		$data = $reader->getSheetByName( 'InstrumentAttributes' );
		if ($data==null) {
			return true;
		}
		if ($this->config->get('export_import_settings_use_attribute_group_id')) {
			if ($this->config->get('export_import_settings_use_attribute_id')) {
				$expected_heading = array( "instrument_id", "attribute_group_id", "attribute_id", "text" );
			} else {
				$expected_heading = array( "instrument_id", "attribute_group_id", "attribute", "text" );
			}
		} else {
			if ($this->config->get('export_import_settings_use_attribute_id')) {
				$expected_heading = array( "instrument_id", "attribute_group", "attribute_id", "text" );
			} else {
				$expected_heading = array( "instrument_id", "attribute_group", "attribute", "text" );
			}
		}
	...
	protected function validateInstrumentFilters( &$reader ) {
		$data = $reader->getSheetByName( 'InstrumentFilters' );
		if ($data==null) {
			return true;
		}
		...
		if ($this->config->get('export_import_settings_use_filter_group_id')) {
			if ($this->config->get('export_import_settings_use_filter_id')) {
				$expected_heading = array( "instrument_id", "filter_group_id", "filter_id" );
			} else {
				$expected_heading = array( "instrument_id", "filter_group_id", "filter" );
			}
		} else {
			if ($this->config->get('export_import_settings_use_filter_id')) {
				$expected_heading = array( "instrument_id", "filter_group", "filter_id" );
			} else {
				$expected_heading = array( "instrument_id", "filter_group", "filter" );
			}
		}
		$expected_multilingual = array();
		return $this->validateHeading( $data, $expected_heading, $expected_multilingual );
	...
	protected function validateUsers( &$reader ) {
		$data = $reader->getSheetByName( 'Users' );
		if ($data==null) {
			return true;
		}
		...
		$expected_heading = array( "user_id", "user_group", "store_id", "firstname", "lastname", "email", "telephone", "fax", "password" );
		if ($exist_salt) {
	...
	protected function validateAddresses( &$reader ) {
		...
		$expected_heading = array( "user_id", "firstname", "lastname", "company" );
		if ($exist_company_id) {
	...
	protected function validateInstrumentIdColumns( &$reader ) {
		$data = $reader->getSheetByName( 'Instruments' );
		if ($data==null) {
			return true;
		}
		...
		// only unique numeric instrument_ids can be used, in ascending order, in worksheet 'Instruments'
		$previous_instrument_id = 0;
		$has_missing_instrument_ids = false;
		$instrument_ids = array();
		$k = $data->getHighestRow();
		for ($i=1; $i<$k; $i+=1) {
			$instrument_id = $this->getCell($data,$i,1);
			if ($instrument_id=="") {
				if (!$has_missing_instrument_ids) {
					$msg = str_replace( '%1', 'Instruments', $this->language->get( 'error_missing_instrument_id' ) );
					$this->log->write( $msg );
					$has_missing_instrument_ids = true;
				}
				$ok = false;
				continue;
			}
			if (!$this->isInteger($instrument_id)) {
				$msg = str_replace( '%2', $instrument_id, str_replace( '%1', 'Instruments', $this->language->get( 'error_invalid_instrument_id' ) ) );
				$this->log->write( $msg );
				$ok = false;
				continue;
			}
			if (in_array( $instrument_id, $instrument_ids )) {
				$msg = str_replace( '%2', $instrument_id, str_replace( '%1', 'Instruments', $this->language->get( 'error_duplicate_instrument_id' ) ) );
				$this->log->write( $msg );
				$ok = false;
			}
			$instrument_ids[] = $instrument_id;
			if ($instrument_id < $previous_instrument_id) {
				$msg = str_replace( '%2', $instrument_id, str_replace( '%1', 'Instruments', $this->language->get( 'error_wrong_order_instrument_id' ) ) );
				$this->log->write( $msg );
				$ok = false;
			}
			$previous_instrument_id = $instrument_id;
		...
		// make sure instrument_ids are numeric entries and are also mentioned in worksheet 'Instruments'
		$worksheets = array( 'AdditionalImages', 'Specials', 'Discounts', 'Rewards', 'InstrumentOptions', 'InstrumentOptionValues', 'InstrumentAttributes', 'InstrumentFilters' );
		foreach ($worksheets as $worksheet) {
			...
			$previous_instrument_id = 0;
			$has_missing_instrument_ids = false;
			$unlisted_instrument_ids = array();
			$k = $data->getHighestRow();
			for ($i=1; $i<$k; $i+=1) {
				$instrument_id = $this->getCell($data,$i,1);
				if ($instrument_id=="") {
					if (!$has_missing_instrument_ids) {
						$msg = str_replace( '%1', $worksheet, $this->language->get( 'error_missing_instrument_id' ) );
						$this->log->write( $msg );
						$has_missing_instrument_ids = true;
					}
					$ok = false;
				...
				if (!$this->isInteger($instrument_id)) {
					$msg = str_replace( '%2', $instrument_id, str_replace( '%1', $worksheet, $this->language->get( 'error_invalid_instrument_id' ) ) );
					$this->log->write( $msg );
					$ok = false;
					continue;
				}
				if (!in_array( $instrument_id, $instrument_ids )) {
					if (!in_array( $instrument_id, $unlisted_instrument_ids )) {
						$unlisted_instrument_ids[] = $instrument_id;
						$msg = str_replace( '%2', $instrument_id, str_replace( '%1', $worksheet, $this->language->get( 'error_unlisted_instrument_id' ) ) );
						$this->log->write( $msg );
						$ok = false;
					}
				}
				if ($instrument_id < $previous_instrument_id) {
					$msg = str_replace( '%2', $instrument_id, str_replace( '%1', $worksheet, $this->language->get( 'error_wrong_order_instrument_id' ) ) );
					$this->log->write( $msg );
					$ok = false;
				}
				$previous_instrument_id = $instrument_id;
	...
	protected function validateUserIdColumns( &$reader ) {
		$data = $reader->getSheetByName( 'Users' );
		if ($data==null) {
			return true;
		}
		...
		// only unique numeric user_ids can be used, in ascending order, in worksheet 'Users'
		$previous_user_id = 0;
		$has_missing_user_ids = false;
		$user_ids = array();
		$k = $data->getHighestRow();
		for ($i=1; $i<$k; $i+=1) {
			$user_id = $this->getCell($data,$i,1);
			if ($user_id=="") {
				if (!$has_missing_user_ids) {
					$msg = str_replace( '%1', 'Users', $this->language->get( 'error_missing_user_id' ) );
					$this->log->write( $msg );
					$has_missing_user_ids = true;
				}
				$ok = false;
				continue;
			}
			if (!$this->isInteger($user_id)) {
				$msg = str_replace( '%2', $user_id, str_replace( '%1', 'Users', $this->language->get( 'error_invalid_user_id' ) ) );
				$this->log->write( $msg );
				$ok = false;
				continue;
			}
			if (in_array( $user_id, $user_ids )) {
				$msg = str_replace( '%2', $user_id, str_replace( '%1', 'Users', $this->language->get( 'error_duplicate_user_id' ) ) );
				$this->log->write( $msg );
				$ok = false;
			}
			$user_ids[] = $user_id;
			if ($user_id < $previous_user_id) {
				$msg = str_replace( '%2', $user_id, str_replace( '%1', 'Users', $this->language->get( 'error_wrong_order_user_id' ) ) );
				$this->log->write( $msg );
				$ok = false;
			}
			$previous_user_id = $user_id;
		...
		// make sure user_ids are numeric entries and are also mentioned in worksheet 'Users'
		...
		foreach ($worksheets as $worksheet) {
			...
			$previous_user_id = 0;
			$has_missing_user_ids = false;
			$unlisted_user_ids = array();
			$k = $data->getHighestRow();
			for ($i=1; $i<$k; $i+=1) {
				$user_id = $this->getCell($data,$i,1);
				if ($user_id=="") {
					if (!$has_missing_user_ids) {
						$msg = str_replace( '%1', $worksheet, $this->language->get( 'error_missing_user_id' ) );
						$this->log->write( $msg );
						$has_missing_user_ids = true;
					}
					$ok = false;
				...
				if (!$this->isInteger($user_id)) {
					$msg = str_replace( '%2', $user_id, str_replace( '%1', $worksheet, $this->language->get( 'error_invalid_user_id' ) ) );
					$this->log->write( $msg );
					$ok = false;
					continue;
				}
				if (!in_array( $user_id, $user_ids )) {
					if (!in_array( $user_id, $unlisted_user_ids )) {
						$unlisted_user_ids[] = $user_id;
						$msg = str_replace( '%2', $user_id, str_replace( '%1', $worksheet, $this->language->get( 'error_unlisted_user_id' ) ) );
						$this->log->write( $msg );
					}
					$ok = false;
				}
				if ($user_id < $previous_user_id) {
					$msg = str_replace( '%2', $user_id, str_replace( '%1', $worksheet, $this->language->get( 'error_wrong_order_user_id' ) ) );
					$this->log->write( $msg );
					$ok = false;
				}
				$previous_user_id = $user_id;
	...
	protected function validateUserGroupColumns( &$reader ) {
		// all user_groups mentioned in the worksheets must be defined
		$worksheets = array( 'Specials', 'Discounts', 'Rewards', 'Users' );
		$ok = true;
		$user_groups = array();
		$user_group_ids = $this->getUserGroupIds();
		foreach ($worksheets as $worksheet) {
			...
			$has_missing_user_groups = false;
			$k = $data->getHighestRow();
			for ($i=1; $i<$k; $i+=1) {
				$user_group = trim($this->getCell($data,$i,2));
				if ($user_group=="") {
					if (!$has_missing_user_groups) {
						$msg = $this->language->get( 'error_missing_user_group' );
						$msg = str_replace( '%1', $worksheet, $msg );
						$this->log->write( $msg );
						$has_missing_user_groups = true;
					}
					$ok = false;
					continue;
				}
				if (!in_array( $user_group, $user_groups )) {
					if (!isset($user_group_ids[$user_group])) {
						$msg = $this->language->get( 'error_invalid_user_group' );
						$msg = str_replace( '%1', $worksheet, str_replace( '%2', $user_group, $msg ) );
						$this->log->write( $msg );
						$ok = false;
						continue;
					}
	...
	protected function validateOptionColumns( &$reader ) {
		...
		// only existing options can be used in 'InstrumentOptions' worksheet
		...
		$k = $data->getHighestRow();
		for ($i=1; $i<$k; $i+=1) {
			$instrument_id = trim($this->getCell($data,$i,1));
			if ($instrument_id=="") {
				continue;
			}
			if ($export_import_settings_use_option_id) {
				...
				if ($option_id=="") {
					if (!$has_missing_options) {
						$msg = str_replace( '%1', 'InstrumentOptions', $this->language->get( 'error_missing_option_id' ) );
						$this->log->write( $msg );
				...
				if (!isset($options[$option_id])) {
					...
					$msg = str_replace( '%1', 'InstrumentOptions', $msg );
					$msg = str_replace( '%2', $option_id, $msg );
					$this->log->write( $msg );
					$ok = false;
					continue;
				}
				$instrument_options[$instrument_id][$option_id] = true;
			} else {
				$option_name = trim($this->getCell($data,$i,2));
				if ($option_name=="") {
					if (!$has_missing_options) {
						$msg = str_replace( '%1', 'InstrumentOptions', $this->language->get( 'error_missing_option_name' ) );
						$this->log->write( $msg );
						$has_missing_options = true;
					}
					$ok = false;
				...
				$instrument_options[$instrument_id][$option_name] = true;
			}
		}
		
		// only existing options and option values can be used in 'InstrumentOptionValues' worksheet
		$data = $reader->getSheetByName( 'InstrumentOptionValues' );
		if ($data==null) {
			return $ok;
		}
		...
		for ($i=1; $i<$k; $i+=1) {
			...
			if ($export_import_settings_use_option_id) {
				...
				if ($option_id=="") {
					if (!$has_missing_options) {
						$msg = str_replace( '%1', 'InstrumentOptionValues', $this->language->get( 'error_missing_option_id' ) );
						$this->log->write( $msg );
						$has_missing_options = true;
					}
					$ok = false;
				...
				if (!isset($options[$option_id])) {
					$msg = $this->language->get( 'error_invalid_option_id' );
					$msg = str_replace( '%1', 'InstrumentOptionValues', $msg );
					$msg = str_replace( '%2', $option_id, $msg );
					$this->log->write( $msg );
					$ok = false;
					continue;
				}
				if (!isset($instrument_options[$instrument_id][$option_id])) {
					$msg = $this->language->get( 'error_invalid_instrument_id_option_id' );
					$msg = str_replace( '%1', 'InstrumentOptionValues', $msg );
					$msg = str_replace( '%2', $instrument_id, $msg );
					$msg = str_replace( '%3', $option_id, $msg );
					$msg = str_replace( '%4', 'InstrumentOptions', $msg );
					$this->log->write( $msg );
					$ok = false;
					continue;
				}
				if ($export_import_settings_use_option_value_id) {
					...
					if ($option_value_id=="") {
						if (!$has_missing_option_values) {
							$msg = str_replace( '%1', 'InstrumentOptionValues', $this->language->get( 'error_missing_option_value_id' ) );
							$this->log->write( $msg );
					...
					if ($option_value_name=="") {
						if (!$has_missing_option_values) {
							$msg = str_replace( '%1', 'InstrumentOptionValues', $this->language->get( 'error_missing_option_value_name' ) );
							$this->log->write( $msg );
							$has_missing_option_values = true;
						}
						$ok = false;
				...
				if ($option_name=="") {
					if (!$has_missing_options) {
						$msg = str_replace( '%1', 'InstrumentOptionValues', $this->language->get( 'error_missing_option_name' ) );
						$this->log->write( $msg );
						$has_missing_options = true;
					}
					$ok = false;
				...
				if (!isset($instrument_options[$instrument_id][$option_name])) {
					$msg = $this->language->get( 'error_invalid_instrument_id_option_name' );
					$msg = str_replace( '%1', 'InstrumentOptionValues', $msg );
					$msg = str_replace( '%2', $instrument_id, $msg );
	...
	protected function validateAttributeColumns( &$reader ) {
		...
		// only existing attribute_groups and attributes can be used in 'InstrumentAttributes' worksheet
		$data = $reader->getSheetByName( 'InstrumentAttributes' );
		if ($data==null) {
			return $ok;
		}
			...
			if ($export_import_settings_use_attribute_group_id) {
				...
				if ($attribute_group_id=="") {
					if (!$has_missing_attribute_groups) {
						$msg = str_replace( '%1', 'InstrumentAttributes', $this->language->get( 'error_missing_attribute_group_id' ) );
						$this->log->write( $msg );
				...
				if (!isset($attribute_groups[$attribute_group_id])) {
					...
					$msg = str_replace( '%1', 'InstrumentAttributes', $msg );
				...
				if ($export_import_settings_use_attribute_id) {
					...
					if ($attribute_id=="") {
						if (!$has_missing_attributes) {
							$msg = str_replace( '%1', 'InstrumentAttributes', $this->language->get( 'error_missing_attribute_id' ) );
							$this->log->write( $msg );
					...
					if ($attribute_name=="") {
						if (!$has_missing_attributes) {
							$msg = str_replace( '%1', 'InstrumentAttributes', $this->language->get( 'error_missing_attribute_name' ) );
							$this->log->write( $msg );
							$has_missing_attributes = true;
						}
						$ok = false;
				...
				if ($attribute_group_name=="") {
					if (!$has_missing_attribute_groups) {
						$msg = str_replace( '%1', 'InstrumentAttributes', $this->language->get( 'error_missing_attribute_group_name' ) );
						$this->log->write( $msg );
						$has_missing_attribute_groups = true;
					}
					$ok = false;
	...
	protected function validateIncrementalOnly( &$reader, $incremental ) {
		...
		$ok = true;
		$worksheets = array( 'Users', 'Addresses' );
		foreach ($worksheets as $worksheet) {
			$data = $reader->getSheetByName( $worksheet );
			if ($data) {
				if (!$incremental) {
	...
	protected function validateUpload( &$reader )
	{
		...
		if (!$this->validateInstruments( $reader )) {
			$this->log->write( $this->language->get('error_instruments_header') );
			$ok = false;
		}
		...
		if (!$this->validateInstrumentOptions( $reader )) {
			$this->log->write( $this->language->get('error_instrument_options_header') );
			$ok = false;
		}
		if (!$this->validateInstrumentOptionValues( $reader )) {
			$this->log->write( $this->language->get('error_instrument_option_values_header') );
			$ok = false;
		}
		if (!$this->validateInstrumentAttributes( $reader )) {
			$this->log->write( $this->language->get('error_instrument_attributes_header') );
			$ok = false;
		}
		if (!$this->validateInstrumentFilters( $reader )) {
			$this->log->write( $this->language->get('error_instrument_filters_header') );
			$ok = false;
		}
		...
		if (!$this->validateUsers( $reader )) {
			$this->log->write( $this->language->get('error_users_header') );
			$ok = false;
		}
		...
		$exist_instrument_options = false;
		$exist_instrument_option_values = false;
		$exist_instruments = false;
		$exist_additional_images = false;
		$exist_specials = false;
		$exist_discounts = false;
		$exist_rewards = false;
		$exist_instrument_attributes = false;
		$exist_instrument_filters = false;
		...
		foreach ($names as $name) {
			...
			if ($name=='Instruments') {
				$exist_instruments = true;
				continue;
			}
			if ($name=='InstrumentOptions') {
				if (!$exist_instruments) {
					// Missing Instruments worksheet, or Instruments worksheet not listed before InstrumentOptions
					$this->log->write( $this->language->get('error_instrument_options') );
					$ok = false;
				}
				$exist_instrument_options = true;
				continue;
			}
			if ($name=='InstrumentOptionValues') {
				if (!$exist_instruments) {
					// Missing Instruments worksheet, or Instruments worksheet not listed before InstrumentOptionValues
					$this->log->write( $this->language->get('error_instrument_option_values') );
					$ok = false;
				}
				if (!$exist_instrument_options) {
					// Missing InstrumentOptions worksheet, or InstrumentOptions worksheet not listed before InstrumentOptionValues
					$this->log->write( $this->language->get('error_instrument_option_values_2') );
					$ok = false;
				}
				$exist_instrument_option_values = true;
				continue;
			}
			if ($name=='AdditionalImages') {
				if (!$exist_instruments) {
					// Missing Instruments worksheet, or Instruments worksheet not listed before AdditionalImages
			...
			if ($name=='Specials') {
				if (!$exist_instruments) {
					// Missing Instruments worksheet, or Instruments worksheet not listed before Specials
			...
			if ($name=='Discounts') {
				if (!$exist_instruments) {
					// Missing Instruments worksheet, or Instruments worksheet not listed before Discounts
			...
			if ($name=='Rewards') {
				if (!$exist_instruments) {
					// Missing Instruments worksheet, or Instruments worksheet not listed before Rewards
			...
			if ($name=='InstrumentAttributes') {
				if (!$exist_instruments) {
					// Missing Instruments worksheet, or Instruments worksheet not listed before InstrumentAttributes
					$this->log->write( $this->language->get('error_instrument_attributes') );
					$ok = false;
				}
				$exist_instrument_attributes = true;
				continue;
			}
			...
			if ($name=='InstrumentFilters') {
				if (!$exist_instruments) {
					// Missing Instruments worksheet, or Instruments worksheet not listed before InstrumentFilters
					$this->log->write( $this->language->get('error_instrument_filters') );
					$ok = false;
				}
				$exist_instrument_filters = true;
				continue;
			}
			...
			if ($name=='Addresses') {
				if (!$exist_users) {
					// Missing Cutomers worksheet, or Users worksheet not listed before Addresses
		...
		if ($exist_instrument_options) {
			if (!$exist_instrument_option_values) {
				// InstrumentOptionValues worksheet also expected after a InstrumentOptions worksheet
				$this->log->write( $this->language->get('error_instrument_option_values_3') );
				$ok = false;
			}
		}
		if ($exist_attribute_groups) {
		...
		if ($exist_users) {
			if (!$exist_addresses) {
				// Addresses worksheet also expected after Users worksheet
		...
		if (!$this->validateInstrumentIdColumns( $reader )) {
			$ok = false;
		}
		...
		if (!$this->validateUserIdColumns( $reader )) {
			$ok = false;
		}

		if (!$this->validateUserGroupColumns( $reader )) {
			$ok = false;
		}
	...
	public function upload( $filename, $incremental=false ) {
		...
		try {
			...
			$this->uploadInstruments( $reader, $incremental, $available_instrument_ids );
			$this->uploadAdditionalImages( $reader, $incremental, $available_instrument_ids );
			$this->uploadSpecials( $reader, $incremental, $available_instrument_ids );
			$this->uploadDiscounts( $reader, $incremental, $available_instrument_ids );
			$this->uploadRewards( $reader, $incremental, $available_instrument_ids );
			$this->uploadInstrumentOptions( $reader, $incremental, $available_instrument_ids );
			$this->uploadInstrumentOptionValues( $reader, $incremental, $available_instrument_ids );
			$this->uploadInstrumentAttributes( $reader, $incremental, $available_instrument_ids );
			$this->uploadInstrumentFilters( $reader, $incremental, $available_instrument_ids );
			...
			$this->uploadUsers( $reader, $incremental, $available_user_ids );
			$this->uploadAddresses( $reader, $incremental, $available_user_ids );
			return true;
		} catch (Exception $e) {
			$errstr = $e->getMessage();
			$errline = $e->getLine();
...
//		}
	...
	protected function getStoreIdsForInstruments() {
		$sql =  "SELECT instrument_id, store_id FROM `".DB_PREFIX."instrument_to_store` ps;";
		$store_ids = array();
		$result = $this->db->query( $sql );
		foreach ($result->rows as $row) {
			$instrumentId = $row['instrument_id'];
			$store_id = $row['store_id'];
			if (!isset($store_ids[$instrumentId])) {
				$store_ids[$instrumentId] = array();
			}
			if (!in_array($store_id,$store_ids[$instrumentId])) {
				$store_ids[$instrumentId][] = $store_id;
			}
		}
		return $store_ids;
	}
	...
	protected function getLayoutsForInstruments() {
		$sql  = "SELECT pl.*, l.name FROM `".DB_PREFIX."instrument_to_layout` pl ";
		$sql .= "LEFT JOIN `".DB_PREFIX."layout` l ON pl.layout_id = l.layout_id ";
		$sql .= "ORDER BY pl.instrument_id, pl.store_id;";
		$result = $this->db->query( $sql );
		$layouts = array();
		foreach ($result->rows as $row) {
			$instrumentId = $row['instrument_id'];
			$store_id = $row['store_id'];
			$name = $row['name'];
			if (!isset($layouts[$instrumentId])) {
				$layouts[$instrumentId] = array();
			}
			$layouts[$instrumentId][$store_id] = $name;
		}
		return $layouts;
	}


	protected function getInstrumentDescriptions( &$languages, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
		// some older versions of OpenCart use the 'instrument_tag' table
		$exist_table_instrument_tag = false;
		$query = $this->db->query( "SHOW TABLES LIKE '".DB_PREFIX."instrument_tag'" );
		$exist_table_instrument_tag = ($query->num_rows > 0);

		// query the instrument_description table for each language
		$instrument_descriptions = array();
		foreach ($languages as $language) {
			$language_id = $language['language_id'];
			$language_code = $language['code'];
			$sql  = "SELECT p.instrument_id, ".(($exist_table_instrument_tag) ? "GROUP_CONCAT(pt.tag SEPARATOR \",\") AS tag, " : "")."pd.* ";
			$sql .= "FROM `".DB_PREFIX."instrument` p ";
			$sql .= "LEFT JOIN `".DB_PREFIX."instrument_description` pd ON pd.instrument_id=p.instrument_id AND pd.language_id='".(int)$language_id."' ";
			if ($exist_table_instrument_tag) {
				$sql .= "LEFT JOIN `".DB_PREFIX."instrument_tag` pt ON pt.instrument_id=p.instrument_id AND pt.language_id='".(int)$language_id."' ";
			}
			if (isset($min_id) && isset($max_id)) {
				$sql .= "WHERE p.instrument_id BETWEEN $min_id AND $max_id ";
			}
			$sql .= "GROUP BY p.instrument_id ";
			$sql .= "ORDER BY p.instrument_id ";
			if (isset($offset) && isset($rows)) {
				$sql .= "LIMIT $offset,$rows; ";
			} else {
				$sql .= "; ";
			...
			$query = $this->db->query( $sql );
			$instrument_descriptions[$language_code] = $query->rows;
		}
		return $instrument_descriptions;
	}


	protected function getInstruments( &$languages, $default_language_id, $instrument_fields, $exist_meta_title, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
		$sql  = "SELECT ";
		$sql .= "  p.instrument_id,";
		...
		$sql .= "FROM `".DB_PREFIX."instrument` p ";
		$sql .= "LEFT JOIN `".DB_PREFIX."instrument_to_category` pc ON p.instrument_id=pc.instrument_id ";
		$sql .= "LEFT JOIN `".DB_PREFIX."url_alias` ua ON ua.query=CONCAT('instrument_id=',p.instrument_id) ";
		...
		$sql .= "LEFT JOIN `".DB_PREFIX."instrument_related` pr ON pr.instrument_id=p.instrument_id ";
		if (isset($min_id) && isset($max_id)) {
			$sql .= "WHERE p.instrument_id BETWEEN $min_id AND $max_id ";
		}
		$sql .= "GROUP BY p.instrument_id ";
		...
		$results = $this->db->query( $sql );
		$instrument_descriptions = $this->getInstrumentDescriptions( $languages, $offset, $rows, $min_id, $max_id );
		foreach ($languages as $language) {
			$language_code = $language['code'];
			foreach ($results->rows as $key=>$row) {
				if (isset($instrument_descriptions[$language_code][$key])) {
					$results->rows[$key]['name'][$language_code] = $instrument_descriptions[$language_code][$key]['name'];
					$results->rows[$key]['description'][$language_code] = $instrument_descriptions[$language_code][$key]['description'];
					if ($exist_meta_title) {
						$results->rows[$key]['meta_title'][$language_code] = $instrument_descriptions[$language_code][$key]['meta_title'];
					}
					$results->rows[$key]['meta_description'][$language_code] = $instrument_descriptions[$language_code][$key]['meta_description'];
					$results->rows[$key]['meta_keyword'][$language_code] = $instrument_descriptions[$language_code][$key]['meta_keyword'];
					$results->rows[$key]['tag'][$language_code] = $instrument_descriptions[$language_code][$key]['tag'];
				} else {
					$results->rows[$key]['name'][$language_code] = '';
					$results->rows[$key]['description'][$language_code] = '';
					if ($exist_meta_title) {
	...
	protected function getAdditionalImages( $min_id=null, $max_id=null, $exist_sort_order=true  ) {
		if ($exist_sort_order) {
			$sql  = "SELECT instrument_id, image, sort_order ";
		} else {
			$sql  = "SELECT instrument_id, image ";
		}
		$sql .= "FROM `".DB_PREFIX."instrument_image` ";
		if (isset($min_id) && isset($max_id)) {
			$sql .= "WHERE instrument_id BETWEEN $min_id AND $max_id ";
		}
		if ($exist_sort_order) {
			$sql .= "ORDER BY instrument_id, sort_order, image;";
		} else {
			$sql .= "ORDER BY instrument_id, image;";
		}
		$result = $this->db->query( $sql );
		return $result->rows;
	}
	...
	protected function getSpecials( $language_id, $min_id=null, $max_id=null ) {
		// Newer OC versions use the 'user_group_description' instead of 'user_group' table for the 'name' field
	...
	protected function populateSpecialsWorksheet( &$worksheet, $language_id, &$price_format, &$box_format, &$text_format, $min_id=null, $max_id=null ) {
		// Set the column widths
		$j = 0;
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('instrument_id')+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('user_group')+1);
		...
		// The actual instrument specials data
		$i += 1;
		$j = 0;
	...
	protected function populateDiscountsWorksheet( &$worksheet, $language_id, &$price_format, &$box_format, &$text_format, $min_id=null, $max_id=null ) {
		...
		// The actual instrument discounts data
		$i += 1;
		$j = 0;
	...
	protected function populateRewardsWorksheet( &$worksheet, $language_id, &$box_format, &$text_format, $min_id=null, $max_id=null ) {
		...
		// The actual instrument rewards data
		$i += 1;
		$j = 0;
	...
	protected function getInstrumentOptions( $min_id, $max_id ) {
		...
		// DB query for getting the instrument options
		if ($exist_po_value) {
			$sql  = "SELECT p.instrument_id, po.option_id, po.value AS option_value, po.required, od.name AS `option` FROM ";
		} else {
			$sql  = "SELECT p.instrument_id, po.option_id, po.option_value, po.required, od.name AS `option` FROM ";
		}
		$sql .= "( SELECT instrument_id ";
		$sql .= "  FROM `".DB_PREFIX."instrument` ";
		if (isset($min_id) && isset($max_id)) {
			$sql .= "  WHERE instrument_id BETWEEN $min_id AND $max_id ";
		}
		$sql .= "  ORDER BY instrument_id ASC ";
		$sql .= ") AS p ";
		$sql .= "INNER JOIN `".DB_PREFIX."instrument_option` po ON po.instrument_id=p.instrument_id ";
		$sql .= "INNER JOIN `".DB_PREFIX."option_description` od ON od.option_id=po.option_id AND od.language_id='".(int)$language_id."' ";
		$sql .= "ORDER BY p.instrument_id ASC, po.option_id ASC";
		$query = $this->db->query( $sql );
		return $query->rows;
	}


	protected function populateInstrumentOptionsWorksheet( &$worksheet, &$box_format, &$text_format, $min_id=null, $max_id=null ) {
		// Set the column widths
		$j = 0;
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('instrument_id')+1);
		if ($this->config->get( 'export_import_settings_use_option_id' )) {
		...
		// The actual instrument options data
		$i += 1;
		$j = 0;
		$instrument_options = $this->getInstrumentOptions( $min_id, $max_id );
		foreach ($instrument_options as $row) {
			$worksheet->getRowDimension($i)->setRowHeight(13);
			$data = array();
			$data[$j++] = $row['instrument_id'];
			if ($this->config->get( 'export_import_settings_use_option_id' )) {
	...
	protected function getInstrumentOptionValues( $min_id, $max_id ) {
		$language_id = $this->getDefaultLanguageId();
		$sql  = "SELECT ";
		$sql .= "  p.instrument_id, pov.option_id, pov.option_value_id, pov.quantity, pov.subtract, od.name AS `option`, ovd.name AS option_value, ";
		...
		$sql .= "  ORDER BY instrument_id ASC ";
		$sql .= ") AS p ";
		$sql .= "INNER JOIN `".DB_PREFIX."instrument_option_value` pov ON pov.instrument_id=p.instrument_id ";
		...
		$sql .= "ORDER BY p.instrument_id ASC, pov.option_id ASC, pov.option_value_id";
		$query = $this->db->query( $sql );
		return $query->rows;
	}


	protected function populateInstrumentOptionValuesWorksheet( &$worksheet, &$price_format, &$box_format, &$weight_format, &$text_format, $min_id=null, $max_id=null ) {
		// Set the column widths
		$j = 0;
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('instrument_id')+1);
		if ($this->config->get( 'export_import_settings_use_option_id' )) {
		...
		// The actual instrument option values data
		$i += 1;
		$j = 0;
		$instrument_option_values = $this->getInstrumentOptionValues( $min_id, $max_id );
		foreach ($instrument_option_values as $row) {
			$worksheet->getRowDimension($i)->setRowHeight(13);
			$data = array();
			$data[$j++] = $row['instrument_id'];
			if ($this->config->get( 'export_import_settings_use_option_id' )) {
	...
	protected function getInstrumentAttributes( &$languages, $min_id, $max_id ) {
		$sql  = "SELECT pa.instrument_id, ag.attribute_group_id, pa.attribute_id, pa.language_id, pa.text ";
		$sql .= "FROM `".DB_PREFIX."instrument_attribute` pa ";
		...
		$sql .= "ORDER BY pa.instrument_id ASC, ag.attribute_group_id ASC, pa.attribute_id ASC";
		$query = $this->db->query( $sql );
		$texts = array();
		foreach ($query->rows as $row) {
			$instrument_id = $row['instrument_id'];
			...
			$language_id = $row['language_id'];
			$text = $row['text'];
			$texts[$instrument_id][$attribute_group_id][$attribute_id][$language_id] = $text;
		}
		$instrument_attributes = array();
		foreach ($texts as $instrument_id=>$level1) {
			foreach ($level1 as $attribute_group_id=>$level2) {
				foreach ($level2 as $attribute_id=>$text) {
					...
					$instrument_attribute['attribute_group_id'] = $attribute_group_id;
					$instrument_attribute['attribute_id'] = $attribute_id;
					$instrument_attribute['text'] = array();
					foreach ($languages as $language) {
						$language_id = $language['language_id'];
						$code = $language['code'];
						if (isset($text[$language_id])) {
							$instrument_attribute['text'][$code] = $text[$language_id];
						} else {
							$instrument_attribute['text'][$code] = '';
						}
					}
	...
	protected function populateInstrumentAttributesWorksheet( &$worksheet, &$languages, $default_language_id, &$box_format, &$text_format, $min_id=null, $max_id=null ) {
		// Set the column widths
		$j = 0;
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('instrument_id')+1);
		if ($this->config->get( 'export_import_settings_use_attribute_group_id' )) {
		...
		// The actual instrument attributes data
		if (!$this->config->get( 'export_import_settings_use_attribute_group_id' )) {
		...
		$i += 1;
		$j = 0;
		$instrument_attributes = $this->getInstrumentAttributes( $languages, $min_id, $max_id );
		foreach ($instrument_attributes as $row) {
			$worksheet->getRowDimension($i)->setRowHeight(13);
			$data = array();
			$data[$j++] = $row['instrument_id'];
			if ($this->config->get( 'export_import_settings_use_attribute_group_id' )) {
	...
	protected function getInstrumentFilters( $min_id, $max_id ) {
		$sql  = "SELECT pf.instrument_id, fg.filter_group_id, pf.filter_id ";
		$sql .= "FROM `".DB_PREFIX."instrument_filter` pf ";
		$sql .= "INNER JOIN `".DB_PREFIX."filter` f ON f.filter_id=pf.filter_id ";
		$sql .= "INNER JOIN `".DB_PREFIX."filter_group` fg ON fg.filter_group_id=f.filter_group_id ";
		if (isset($min_id) && isset($max_id)) {
			$sql .= "WHERE instrument_id BETWEEN $min_id AND $max_id ";
		}
		$sql .= "ORDER BY pf.instrument_id ASC, fg.filter_group_id ASC, pf.filter_id ASC";
		$query = $this->db->query( $sql );
		$instrument_filters = array();
		foreach ($query->rows as $row) {
			$instrument_filter = array();
			$instrument_filter['instrument_id'] = $row['instrument_id'];
			$instrument_filter['filter_group_id'] = $row['filter_group_id'];
			$instrument_filter['filter_id'] = $row['filter_id'];
			$instrument_filters[] = $instrument_filter;
		}
		return $instrument_filters;
	}


	protected function populateInstrumentFiltersWorksheet( &$worksheet, &$languages, $default_language_id, &$box_format, &$text_format, $min_id=null, $max_id=null ) {
		// Set the column widths
		$j = 0;
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('instrument_id')+1);
		if ($this->config->get( 'export_import_settings_use_filter_group_id' )) {
		...
		// The actual instrument filters data
		if (!$this->config->get( 'export_import_settings_use_filter_group_id' )) {
			$filter_group_names = $this->getFilterGroupNames( $default_language_id );
		}
		if (!$this->config->get( 'export_import_settings_use_filter_id' )) {
		...
		$i += 1;
		$j = 0;
		$instrument_filters = $this->getInstrumentFilters( $min_id, $max_id );
		foreach ($instrument_filters as $row) {
			$worksheet->getRowDimension($i)->setRowHeight(13);
			$data = array();
			$data[$j++] = $row['instrument_id'];
			if ($this->config->get( 'export_import_settings_use_filter_group_id' )) {
	...
	protected function getUsers( $exist_custom_field, $exist_salt, $exist_safe, $exist_token, $exist_code, $offset=null, $rows=null, $min_id=null, $max_id=null ) {
		// Newer OC versions use the 'user_group_description' instead of 'user_group' table for the 'name' field
		$exist_table_user_group_description = false;
		$query = $this->db->query( "SHOW TABLES LIKE '".DB_PREFIX."user_group_description'" );
		$exist_table_user_group_description = ($query->num_rows > 0);
		...
		if ($exist_table_user_group_description) {
			$sql  = "SELECT c.*, cgd.name AS user_group FROM `".DB_PREFIX."user` c ";
			$sql .= "INNER JOIN `".DB_PREFIX."user_group_description` cgd ON cgd.user_group_id=c.user_group_id AND cgd.language_id='".(int)$language_id."' ";
		} else {
			$sql  = "SELECT c.*, cg.name AS user_group FROM `".DB_PREFIX."user` c ";
			$sql .= "INNER JOIN `".DB_PREFIX."user_group` cg ON cg.user_group_id=c.user_group_id ";
		}
		if (isset($min_id) && isset($max_id)) {
			$sql .= "WHERE c.user_id BETWEEN $min_id AND $max_id ";
		}
		$sql .= "GROUP BY c.`user_id` ";
		$sql .= "ORDER BY c.`user_id` ASC ";
		if (isset($offset) && isset($rows)) {
			$sql .= "LIMIT $offset,$rows; ";
		} else {
			$sql .= "; ";
	...
	protected function populateUsersWorksheet( &$worksheet, &$box_format, &$text_format, $offset=null, $rows=null, &$min_id=null, &$max_id=null ) {
		// Some fields are only available in newer Opencart versions
		$sql = "SHOW COLUMNS FROM `".DB_PREFIX."user` LIKE 'custom_field'";
		$query = $this->db->query( $sql );
		$exist_custom_field = ($query->num_rows > 0) ? true : false;
		...
		// Set the column widths
		$j = 0;
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('user_id')+1);
		$worksheet->getColumnDimensionByColumn($j++)->setWidth(strlen('user_group')+1);
		...
		// The actual users data
		$i += 1;
		$j = 0;
		$users = $this->getUsers( $exist_custom_field, $exist_salt, $exist_safe, $exist_token, $exist_code, $offset, $rows, $min_id, $max_id );
		$len = count($users);
		$min_id = $users[0]['user_id'];
		$max_id = $users[$len-1]['user_id'];
		foreach ($users as $row) {
			$worksheet->getRowDimension($i)->setRowHeight(26);
			$data = array();
			$data[$j++] = $row['user_id'];
			$data[$j++] = $row['user_group'];
			$data[$j++] = $row['store_id'];
	...
	protected function getAddresses( $min_id, $max_id ) {
		...
		$sql .= "INNER JOIN `".DB_PREFIX."user` cu ON cu.user_id=a.user_id ";
		if (isset($min_id) && isset($max_id)) {
			$sql .= "  WHERE a.user_id BETWEEN $min_id AND $max_id ";
		}
		$sql .= "  ORDER BY a.user_id ASC, a.address_id ASC;";
		$this->log->write( $sql );
		$query = $this->db->query( $sql );
		return $query->rows;
	}
	...
	public function getMaxInstrumentId() {
		$query = $this->db->query( "SELECT MAX(instrument_id) as max_instrument_id FROM `".DB_PREFIX."instrument`" );
		if (isset($query->row['max_instrument_id'])) {
			$max_id = $query->row['max_instrument_id'];
		} else {
			$max_id = 0;
		}
		return $max_id;
	...
	public function getMinInstrumentId() {
		$query = $this->db->query( "SELECT MIN(instrument_id) as min_instrument_id FROM `".DB_PREFIX."instrument`" );
		if (isset($query->row['min_instrument_id'])) {
			$min_id = $query->row['min_instrument_id'];
		} else {
			$min_id = 0;
		}
		return $min_id;
	...
	public function getCountInstrument() {
		$query = $this->db->query( "SELECT COUNT(instrument_id) as count_instrument FROM `".DB_PREFIX."instrument`" );
		if (isset($query->row['count_instrument'])) {
			$count = $query->row['count_instrument'];
		} else {
			$count = 0;
		}
		return $count;
	...
	public function getMaxUserId() {
		$query = $this->db->query( "SELECT MAX(user_id) as max_user_id FROM `".DB_PREFIX."user`" );
		if (isset($query->row['max_user_id'])) {
			$max_id = $query->row['max_user_id'];
		} else {
			$max_id = 0;
		}
		return $max_id;
	...
	public function getMinUserId() {
		$query = $this->db->query( "SELECT MIN(user_id) as min_user_id FROM `".DB_PREFIX."user`" );
		if (isset($query->row['min_user_id'])) {
			$min_id = $query->row['min_user_id'];
		} else {
			$min_id = 0;
		}
		return $min_id;
	...
	public function getCountUser() {
		$query = $this->db->query( "SELECT COUNT(user_id) as count_user FROM `".DB_PREFIX."user`" );
		if (isset($query->row['count_user'])) {
			$count = $query->row['count_user'];
		} else {
			$count = 0;
		}
		return $count;
	...
	public function download( $export_type, $offset=null, $rows=null, $min_id=null, $max_id=null) {
		...
		try {
			...
			switch ($export_type) {
				...
				case 'p':
					// creating the Instruments worksheet
					$workbook->setActiveSheetIndex($worksheet_index++);
					$worksheet = $workbook->getActiveSheet();
					$worksheet->setTitle( 'Instruments' );
					$this->populateInstrumentsWorksheet( $worksheet, $languages, $default_language_id, $price_format, $box_format, $weight_format, $text_format, $offset, $rows, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 1, 2 );
					...
					// creating the InstrumentOptions worksheet
					$workbook->createSheet();
					$workbook->setActiveSheetIndex($worksheet_index++);
					$worksheet = $workbook->getActiveSheet();
					$worksheet->setTitle( 'InstrumentOptions' );
					$this->populateInstrumentOptionsWorksheet( $worksheet, $box_format, $text_format, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 1, 2 );

					// creating the InstrumentOptionValues worksheet
					$workbook->createSheet();
					$workbook->setActiveSheetIndex($worksheet_index++);
					$worksheet = $workbook->getActiveSheet();
					$worksheet->setTitle( 'InstrumentOptionValues' );
					$this->populateInstrumentOptionValuesWorksheet( $worksheet, $price_format, $box_format, $weight_format, $text_format, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 1, 2 );

					// creating the InstrumentAttributes worksheet
					$workbook->createSheet();
					$workbook->setActiveSheetIndex($worksheet_index++);
					$worksheet = $workbook->getActiveSheet();
					$worksheet->setTitle( 'InstrumentAttributes' );
					$this->populateInstrumentAttributesWorksheet( $worksheet, $languages, $default_language_id, $box_format, $text_format, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 1, 2 );

					// creating the InstrumentFilters worksheet
					if ($this->existFilter()) {
						$workbook->createSheet();
						$workbook->setActiveSheetIndex($worksheet_index++);
						$worksheet = $workbook->getActiveSheet();
						$worksheet->setTitle( 'InstrumentFilters' );
						$this->populateInstrumentFiltersWorksheet( $worksheet, $languages, $default_language_id, $box_format, $text_format, $min_id, $max_id );
						$worksheet->freezePaneByColumnAndRow( 1, 2 );
					}
					break;
				...
				case 'u':
					...
					$this->populateUsersWorksheet( $worksheet, $box_format, $text_format, $offset, $rows, $min_id, $max_id );
					$worksheet->freezePaneByColumnAndRow( 1, 2 );
